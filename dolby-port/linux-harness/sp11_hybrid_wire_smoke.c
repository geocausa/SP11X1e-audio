...
typedef struct HGuid { uint32_t d1; uint16_t d2,d3; uint8_t d4[8]; } HGuid;
typedef struct HFakeUnknown HFakeUnknown;
typedef struct { int32_t(*qi)(HFakeUnknown*,const HGuid*,void**); uint32_t(*addref)(HFakeUnknown*); uint32_t(*release)(HFakeUnknown*); } HFakeVtbl;
struct HFakeUnknown { HFakeVtbl *vt; uint32_t refs; };
static int32_t hfake_qi(HFakeUnknown*o,const HGuid*i,void**out){(void)i;if(!out)return (int32_t)0x80004003u;*out=o;o->refs++;return 0;}
static uint32_t hfake_addref(HFakeUnknown*o){return ++o->refs;}
static uint32_t hfake_release(HFakeUnknown*o){if(o->refs)o->refs--;return o->refs;}
static HFakeVtbl g_hfvt={hfake_qi,hfake_addref,hfake_release};
static HFakeUnknown g_hftm={&g_hfvt,1};
static int32_t h_CoCreateFreeThreadedMarshaler(void *outer, void **out){(void)outer;if(!out)return (int32_t)0x80004003u;g_hftm.refs++;*out=&g_hftm;return 0;}
static void *h_CoTaskMemAlloc(size_t n){return malloc(n?n:1);}
static void *h_malloc(size_t n){return malloc(n?n:1);}
static void h_free(void*p){free(p);}
static int h_callnewh(size_t n){(void)n;return 0;}
static void hpatch(Sp11PeImage*i,uint32_t rva,void*f){*(uintptr_t*)(i->base+rva)=(uintptr_t)f;}
static int32_t hqi(void *obj,const HGuid*iid,void**out){void **vt=*(void***)obj; return ((int32_t(*)(void*,const HGuid*,void**))vt[0])(obj,iid,out);}
...
int main(int ac,char**av){
 ...
 /* Wire the original HRTF WinRT encoder to this original DAP object. */
 if(ac<3){fprintf(stderr,"need HRTF dll as argv[2]\n");return 9;}
 Sp11PeImage hi; if(sp11_pe_load(&hi,av[2])){fprintf(stderr,"HRTF load failed\n");return 10;}
 hpatch(&hi,0x18000,(void*)h_CoCreateFreeThreadedMarshaler);
 hpatch(&hi,0x18008,(void*)h_CoTaskMemAlloc);
 hpatch(&hi,0x18208,(void*)h_callnewh);
 hpatch(&hi,0x18218,(void*)h_malloc);
 hpatch(&hi,0x18220,(void*)h_free);
 uint32_t hflags=1; void *hfactory=NULL,*hinst=NULL;
 typedef int32_t(*HFactoryCtor)(uint32_t*,void*,const HGuid*,void**);
 HFactoryCtor hfc=(HFactoryCtor)(hi.base+0x3c10);
 int32_t hhr=hfc(&hflags,hi.base+0x19fa8,(const HGuid*)(hi.base+0x1a150),&hfactory);
 fprintf(stderr,"HRTF factory hr=%#x factory=%p\n",(unsigned)hhr,hfactory);
 if(hhr<0||!hfactory)return 11;
 void **hfvt=*(void***)hfactory; hhr=((int32_t(*)(void*,void**))hfvt[6])(hfactory,&hinst);
 fprintf(stderr,"HRTF activate hr=%#x instance=%p\n",(unsigned)hhr,hinst); if(hhr<0||!hinst)return 12;
 HGuid g_setup={0xdc57ddb4,0xe086,0x49ec,{0xb1,0x3d,0xec,0xcd,0xd5,0x12,0x99,0x0c}};
 HGuid g_asar={0xdeff1192,0xf581,0x4d77,{0x9c,0x1b,0x3e,0x59,0x6b,0x0c,0xa9,0x89}};
 void *hsetup=NULL,*hasar=NULL; int32_t qhr=hqi(hinst,&g_setup,&hsetup); int32_t ahr=hqi(hinst,&g_asar,&hasar);
 fprintf(stderr,"HRTF QI setup hr=%#x ptr=%p delta=%td vt_rva=%#llx; asar hr=%#x ptr=%p vt_rva=%#llx\n",(unsigned)qhr,hsetup,hsetup?(char*)hsetup-(char*)hinst:0,hsetup?(unsigned long long)((uint8_t*)*(void**)hsetup-hi.base):0,(unsigned)ahr,hasar,hasar?(unsigned long long)((uint8_t*)*(void**)hasar-hi.base):0);
 if(qhr<0||!hsetup)return 13;
 void **hsvt=*(void***)hsetup;
 fprintf(stderr,"HRTF setup slot3_rva=%#llx slot4_rva=%#llx\n",(unsigned long long)((uint8_t*)hsvt[3]-hi.base),(unsigned long long)((uint8_t*)hsvt[4]-hi.base));
 int32_t shr=((int32_t(*)(void*,void*))hsvt[3])(hsetup,out);
 fprintf(stderr,"HRTF SetEncoderEngine(DAP original) hr=%#x\n",(unsigned)shr);
 if(shr<0)return 14;
 fprintf(stderr,"HYBRID_WIRE_RESULT PASS\n"); return 0;
 uint8_t *iface=(uint8_t*)out+8; uint8_t host[0x200] __attribute__((aligned(16))); memset(host,0,sizeof(host));
 fprintf(stderr,"INIT pre phase=%u ready=%u host=%p\n",iface[0x2e0],iface[0x2e1],host);
 int32_t ir=((InitializeFn)sp11_pe_ptr_for_va(&img,INITIALIZE_VA))(iface,NULL,host);
 fprintf(stderr,"INIT returned hr=%#x phase=%u ready=%u hostptr=%p\n",(unsigned)ir,iface[0x2e0],iface[0x2e1],*(void**)(iface+0x1a8));
 if(ir>=0){iface[0x2e0]=1;fprintf(stderr,"DIAGNOSTIC force phase=1 for full Configure path\n");}
 uint32_t cfg[8]={1,256,2,0,0,2,0,0};
 fprintf(stderr,"CONFIG pre ready=%u phase=%u cfg={%u,%u,%u,%u,%u,%u,%u,%u}\n",iface[0x2e1],iface[0x2e0],cfg[0],cfg[1],cfg[2],cfg[3],cfg[4],cfg[5],cfg[6],cfg[7]);
 int32_t cr=(int32_t)sp11_win_call4(sp11_pe_ptr_for_va(&img,CONFIGURE_VA),wt.teb,(uint64_t)iface,(uint64_t)cfg,sizeof(cfg),0);
 fprintf(stderr,"CONFIG returned hr=%#x ready=%u phase=%u dap=%p aide=%p oar=%p vlldp=%p frame=%u channels=%u total=%u\n",(unsigned)cr,iface[0x2e1],iface[0x2e0],*(void**)(iface+0x538),*(void**)(iface+0x648),*(void**)(iface+0x690),*(void**)(iface+0x740),*(uint32_t*)(iface+0x30),*(uint32_t*)(iface+0x40),*(uint32_t*)(iface+0x4c));
 return cr<0?4:0;
}
