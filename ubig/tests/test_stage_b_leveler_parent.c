...
#define W 20u
#define PR 4u
static uint32_t rng=0x58990a55u;
    ...
    for(unsigned n=0;n<8000;n++){
        for(unsigned r=0;r<R;r++) {
            for(unsigned i=0;i<W;i++) {
                inrows[r][i]=fr(-.18f,.16f);
                outrows[r][i]=fr(-.08f,.08f);
            }
        }
        for(unsigned i=0;i<W;i++)source_vec[i]=fr(.08f,1.05f);
        source.gate=(n%19u==0u)?-.01f:fr(.015f,.12f);
        UbigStageBLevelerParentControl ctl={fr(-.08f,.08f),fr(-.24f,.06f),fr(-.24f,.06f),fr(.3f,.8f),fr(.15f,.95f),fr(.15f,.95f),(n%3u)==0u,(n%29u)==0u,(n%7u)==0u,n&1u};
        float y=ubig_stage_b_leveler_parent_process(&state,&cfg,&tuning,&ctl,prev_curve,tmpl_curve,0,&source,&in,&out,tele);
        h=h64(h,&y,4);h=h64(h,inrows,sizeof inrows);h=h64(h,outrows,sizeof outrows);h=h64(h,tele,sizeof tele);
    }
    UbigStageBLevelerLookupState lc=lookup_state;lc.transition_state=lc.cubic_state=0;UbigStageBLevelerRowState rc=life;rc.previous=rc.current=0;UbigStageBLevelerState wc=writer;wc.primary=wc.secondary=0;UbigStageBLevelerAdaptiveState ac=adaptive_state;ac.fast=ac.slow=0;
    h=h64(h,matrix_state,sizeof matrix_state);h=h64(h,transition_state,sizeof transition_state);h=h64(h,&lc,sizeof lc);h=h64(h,lookup_ts,sizeof lookup_ts);h=h64(h,lookup_cs,sizeof lookup_cs);h=h64(h,&rc,sizeof rc);h=h64(h,life_prev,sizeof life_prev);h=h64(h,life_cur,sizeof life_cur);h=h64(h,&producer_state,sizeof producer_state);h=h64(h,&wc,sizeof wc);h=h64(h,primary_v,sizeof primary_v);h=h64(h,secondary_v,sizeof secondary_v);for(unsigned r=0;r<PR;r++){h=h64(h,&primary[r].scalar,8);h=h64(h,&secondary[r].scalar,8);}h=h64(h,&ac,sizeof ac);h=h64(h,adaptive_fast,sizeof adaptive_fast);h=h64(h,adaptive_slow,sizeof adaptive_slow);
    if(h!=0x5914caceb8261553ULL){
        fprintf(stderr,"Stage-B Leveler parent hash %016llx\n",(unsigned long long)h);
        return 2;
    }
    puts("PASS Stage-B deployed stereo Leveler parent regression");
    return 0;
}
