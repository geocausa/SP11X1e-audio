...
static uint32_t rng=0x29f48b61u;
...
int main(void){
    float row0[32],row1[32],features[UBIG_STAGE_B_RT_SLOPE_FEATURES];
    UbigStageBRtSlope32 workspace;uint64_t h=1469598103934665603ULL;
    for(unsigned t=0;t<100000;t++){
        ...
        ubig_stage_b_rt_slope32_prepare(row0,row1,&workspace);
        ubig_stage_b_rt_slope32_features(&workspace,features);
        h=h64(h,&workspace,sizeof workspace);h=h64(h,features,sizeof features);
    }
    if(h!=0xcf367535f84a8a3bULL){fprintf(stderr,"Stage-B slope32-features hash %016llx\n",(unsigned long long)h);return 2;}
    puts("PASS Stage-B RT slope32-feature regression");return 0;
}
