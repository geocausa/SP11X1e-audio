...
The remaining core/state constructor has now been removed from the deployed SP11 integration path. A destructive liveness sweep over its 298,937-byte persistent image first zeroed each 4 KiB page independently, then all dead pages simultaneously, and recursively refined the surviving regions through 256-byte, 64-byte and 16-byte granularity. The complete seven-profile and live-switch contract observes only **46 x 16-byte blocks = 736 bytes** of the pre-profile constructor image; **298,201 / 298,937 bytes** may be zero simultaneously without changing output.

Those 736 bytes normalize cleanly into allocation-relative internal pointers, a small set of scalar/count fields, and references to immutable DLL tables. A private source initializer now zeros the persistent arena and reconstructs only that live image, rebasing internal references and resolving immutable table references directly. Reference core constructor `0x18002BE18` is overwritten with `BRK` and never called. All seven fixed-profile plugin tests remain bit-exact, the deterministic 16-second profile sweep remains **0 / 1,536,000 differences**, and the randomized 30-second 33-switch stress remains **0 / 2,880,000 differences**. The remaining VR blob dependency at this boundary is immutable table data plus cleanup/control scaffolding, not core-construction code.
