...
A stronger private proof removes `DolbyAPOVR.dll` loading entirely. The five caller-owned control descriptors plus the 76-float projection LUT are supplied by a small external proof pack (40,364 bytes payload; 40,388 bytes including its fixed header), while the already recovered cold-state/tuning data remains source-owned inside the private harness for now. With `SP11_DOLBY_VR_DLL` forced to a nonexistent path, all seven 200,000-frame profile tests still produce hash `6ea2c4cde4cbfb83`, the 16-second profile switch remains **0 / 1,536,000 differences**, and the 30-second randomized 33-switch stress remains **0 / 2,880,000 differences** with matching controller/scheduler counters and zero relevant fallbacks. `strace` over a standalone run records **no DolbyAPOVR open attempt** and only the external control-pack read.

This establishes that the deployed source-owned SP11 VR realtime path has no remaining executable dependency on the Windows VR DLL. It is not yet the public deployment finish line: proof-only recovered coefficient/state bytes still need to be consolidated behind an external data-pack ABI (or replaced by independently generated equivalents) before the repository can ship the same no-DLL path without embedding proprietary payloads.


### Stage-B recovered payload fully externalized

The private no-DLL adapter has now been scrubbed of the remaining recovered Stage-B byte-array initializers. A versioned external proof pack carries the 34 x 64-byte cold-state regions, complete `0xC0` cold config image, history filter/phase/history coefficients, band endpoints, feature boundaries, `0x90` deep-controller config and lane weights, five 500-term control descriptors, and the 76-float projection LUT. The fixed-layout v2 pack is **46,388 bytes total** (68-byte header + 46,320-byte payload; SHA-256 `327803e94ab1072d2e5fcbc60974779632dbe8568ae79e54ee5587a720804b27`). The adapter source now declares mutable semantic buffers and populates them exclusively from the owner-supplied pack before constructing the source-owned state graph.

With the VR DLL path forced nonexistent, the pack-only build remains exact on all seven 200,000-frame profiles (hash `6ea2c4cde4cbfb83`), deterministic live switching remains **0 / 1,536,000 differences**, and randomized 33-switch stress remains **0 / 2,880,000 differences** with paired scheduler/controller topology unchanged. Standalone `strace` again records no `DolbyAPOVR.dll` open. Deliberately removing the pack makes instantiation fail rather than silently falling back, so the remaining proprietary-data dependency is explicit and auditable.

This is the first private Stage-B integration proof whose source contains the recovered behavior but not the recovered Stage-B payload bytes and whose runtime loads no VR DLL. The next promotion step is deployment plumbing for an owner-supplied pack plus a reproducible local pack-generation/migration path; the pack itself remains outside Git.
...
The lean proof remains bit-exact against the immediately preceding pack-only checkpoint. All seven 200,000-frame profile runs retain hash `6ea2c4cde4cbfb83` and the same `781 / 3124 / 195 / 39 / 39` Stage-B cadence with zero relevant fallbacks. The deterministic profile-switch differential remains **0 / 1,536,000 differences** and the randomized 33-switch stress remains **0 / 2,880,000 differences**. A standalone `strace` run with both historical VR and VLLDP DLL environment paths forced to nonexistent files records zero vendor-DLL opens and still passes the complete plugin regression.

The proof source shrank from 1,731 lines / 120,151 bytes to 1,475 lines / 104,846 bytes, and the built proof shared object from 1,056,912 to 740,256 bytes. Explicit user-supplied Custom/GEQ target programming remains outside this proven built-in-profile contract: instead of reaching a removed PE helper, the lean proof now rejects a non-flat custom GEQ at initialization. The archived private proof source SHA-256 is `d6d80dfb7aafe4660491a00e105ecdc77a5565b5d4991ef6c1476c3dedb418a5`.
