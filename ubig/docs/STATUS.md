...
### Stage-B Custom/GEQ semantic control closure (`0x4C560` / `0x4C8E8` / `0x569A0`)

The separately tracked non-flat Custom path is now represented by source-owned semantics. `ubig_stage_b_rt_band_control_map_prepare()` reconstructs the 20-point integer-frequency interpolation grid from `0x18004C560`; a direct reference differential passes **500,000 randomized calls bit-exact**. `ubig_stage_b_rt_band_target_apply()` reconstructs the clamped integer-target interpolation and Q15-style conversion from `0x18004C8E8`; a direct differential passes **1,000,000 randomized calls bit-exact**. The combined public regression hash is `5bc02567a90e1c28`.

The runtime auxiliary-band application at `0x1800569A0` is native as `ubig_stage_b_rt_band_aux_apply()`. It adds the mapped per-band offset to every active analysis/output row, floors the analysis side at -1, and optionally accumulates the exact floor-scaled meter contribution. A direct reference differential passes **1,000,000 randomized calls bit-exact**; public regression hash: `b3edc401cfc13040`. The native late-pipeline composition now executes this optional adjustment at the reference ordering point, immediately after band analysis and before linked/deep processing; its expanded public lifecycle hash is `1d740803e731f73e`.

The private corrected-v3 integration also reconstructs the GEQ dirty-apply bridge: mapped integer targets become the runtime offset vector with the exact `0x39fc0fc1` scale, and the runtime-enable state is propagated before the next block. A non-flat 20-band curve therefore produces non-flat audio rather than collapsing to the built-in path. Native-vs-reference-mapper integration is exact for a fixed 200,000-frame Custom run, **10 additional randomized Custom curves**, the deterministic all-profile transition sweep (**0 / 1,536,000 differences**) and the 30-second randomized 33-switch stress (**0 / 2,880,000 differences**) while preserving the same native Stage-B scheduler/controller topology. The raw reference `0x56B80` fallback is not used as a warm-state oracle because the semantic native wrapper intentionally no longer maintains every obsolete raw field required by that legacy entry.
...
This target is deliberately not installed by `all` and does not alter the Golden PipeWire graph. A tracked `deploy/ubig-candidate` wrapper now supplies the disposable promotion plumbing: `prepare.sh` rebuilds/tests/audits and stages the candidate plus matching volume/transaction/MSIIR helper copies without touching the active graph; `switch.sh activate` snapshots the exact active Golden PipeWire fragment, preserves the established node names and endpoint-volume ordering, overrides only candidate service `ExecStart` paths, opts those helpers into `ubig-control-v2`, and restores `MemoryDenyWriteExecute=yes`; `switch.sh rollback` restores the saved fragment and removes only candidate-specific drop-ins/control state. An isolated fake-systemd lifecycle test performed prepare → activate → rollback and recovered the original Golden fragment byte-for-byte.

The first real SP11 M6 disposable smoke is now passing. The active filter-chain mapped only the UbiG candidate (no Windows bridge or Dolby DLL), W^X was enforced, the v2 endpoint-volume helper queued postgain `-365` before the first stream, and the native candidate synchronized to the Movie cold profile and acknowledged it on first processing. A 480,000-frame silence run exercised `Movie → Game → Custom(non-flat) → Movie` with main generation/ack `3/3` while postgain remained independently `1/1`; later 144,000- and 96,000-frame runs exercised mute/unmute and idle/wake with the filter-chain PID unchanged and visible volume restored to `0.22`. No filter-chain xrun/NaN/crash marker appeared. The only kernel status-3 record was the already-known graph-start `APM_CMD_SET_CFG 0x01001006` AR_EUNSUPPORTED event followed by the explicit Qualcomm-GSL continue path and successful protected-stage setup. Reviewed evidence: `artifacts/reviewed/2026-08-22-ubig-native-candidate-live-smoke.json`. Remaining M6 work is non-silent/acoustic validation, volume/seek/repeated-playback sweeps, PA/protection telemetry comparison and >8-hour stability.

The next live M6 lifecycle gate also passes. With exact endpoint DSP mute asserted, persisted visible-sink UI points `0.15`, `0.25`, and `0.40` drove the recovered Windows final-volume transactions to GainSteps `1`, `3`, and `12`; restoring the original UI scalar returned `pw-dump` exactly to raw channel gain `0.010282` and mute false. UbiG postgain request/ack stayed `1/1` throughout, proving that ordinary live endpoint volume remains downstream-only for the same candidate generation. Twenty independent 9,600-frame stream open/close cycles preserved the same filter-chain PID and control state with no new fault markers. A separate five-second nonzero two-tone stereo signal was processed live under exact endpoint mute, again without PID/control drift or new faults, exercising the real arithmetic path without producing physical output. Fourteen protected-graph GET_CFG replies since activation all decode accepted with the expected two-speaker SP/SPVI static contract. Reviewed evidence: `artifacts/reviewed/2026-08-22-ubig-native-candidate-m6-lifecycle-v2.json`.

The bounded per-amplifier WSA observer was then explicitly armed for 12 samples/side and reset to zero after the run. Under endpoint DSP mute, sequence 0 captured the pre-PA startup state; every active sequence 1..11 on both amplifiers matches the reviewed Golden common-readback contract exactly: PA `0x01`, status `0x2f/0x00`, errors/interrupts zero, failed-read mask zero, current-limit `0x44` code 17 with override disabled, WAVG `0x00`, CPS local control `0x00`. Both sides produced independent changing ADC/temperature/VBAT raw words. This closes the bounded active-status/fault subgate without claiming audible-power telemetry parity. Reviewed evidence: `artifacts/reviewed/2026-08-22-ubig-native-candidate-wsa-protection-observer.json`.
...
The proprietary `0x18002BE18` constructor body is not required at runtime, but the earlier compact cold-state liveness result is **invalidated**. Its acceptance oracle compared against an accidentally all-zero PCM path, so matching call counts did not make the resulting 34 x 64-byte seed a valid audio-equivalence proof. The historical 2,176-byte seed is retained only as failed-proof evidence and must not be used as a deployment boundary.

The corrected nonzero checkpoint deliberately restores the full **298,937-byte** initialized cold snapshot as external owner-supplied data, rebases allocation-relative pointers, and rebinds live caller-owned data semantically. This is conservative by design: no smaller cold-state seed is currently accepted until reduction is repeated against the nonzero oracle and, where relevant, semantic state/cadence guards. The constructor entry itself still does not execute.


### Stage-B cold core data detachment

The 14 PE-relative references in the hardened cold-core checkpoint have now been split into semantic caller-owned data rather than constructor-owned DLL state. The top 48-kHz configuration object is reconstructed as a complete source-owned `0xC0` object; destructive testing proves all of its otherwise inherited PE pointers dead simultaneously once the native history/filter, band-grid and downstream paths are active. The live history-filter coefficients (640 floats), phase table (128 floats), two 22-float history coefficient vectors, 20 band endpoints and 9 feature boundaries are redirected to source-owned proof storage and the original DLL copies are zeroed.

The remaining deep-controller pointer at raw core `+0x414D0` is likewise detached. Its active semantic portion is a `0x90`-byte tuning object plus a 20-float lane-weight vector; a private source-owned reconstruction survives with both original DLL regions zeroed. All seven fixed profiles keep the exact `781 / 3124 / 195 / 39 / 39` parent/controller/upper/lower-A/lower-B cadence and zero relevant fallbacks, the 16-second sweep remains **0 / 1,536,000 differences**, the 30-second randomized stress remains **0 / 2,880,000 differences**, and five fresh-ASLR runs remain exact.

The final five cold-core PE references are the already-documented caller-owned control models: four primary 500-term descriptors and one 500-term secondary descriptor. A private boundary proof copies each complete `8012`-byte descriptor into external model storage, redirects the cold graph to those buffers, then zeroes all five original DLL model regions. Fixed-profile output, live switching and the full observed hot-call topology remain exact. This establishes that the cold constructor/state graph itself has **no required direct PE pointer dependency**. UbiG does **not** embed these proprietary model bytes; deployment must supply equivalent caller-owned model data through the existing semantic descriptor interface. The five source-image payloads total 40,060 bytes and are tracked privately only by integrity hashes.


### Stage-B post-constructor tuning-data reduction

The universal-analysis projection lookup is now detached from the mapped VR image in the private de-blob proof. Its full 76-float caller-owned LUT is copied to source-owned proof storage, `g_projection_lut` is rebound there, and the original `0x180269160` table is zeroed before audio. All seven 200,000-frame fixed-profile runs preserve the exact controller cadence (`3124` analysis updates, `195` upper scheduler updates and `39/39` lower-A/lower-B updates), the deterministic switch differential remains **0 / 1,536,000**, and the randomized 33-switch stress remains **0 / 2,880,000** with paired counter signatures unchanged.

Two larger groups of historical tuning bindings are now proven unnecessary on the shipped source-owned route rather than needing extraction. The Leveler wrapper entry is still reached once per Stage-B block (`781` calls in the fixed-profile corpus), but all shipped profiles keep its three enable flags clear, so the native wrapper returns before dereferencing lookup/inverse/cubic/offset/threshold/band-weight/tail/curve tuning. The multiband tuning adapter is never entered (`54nat=0`) on the current deployed parent path. Removing both PE tuning-binding groups entirely preserves all seven fixed profiles and both long live-switch differentials with zero relevant fallbacks. This proof removes those data bindings from the deployed contract without claiming the dormant optional algorithms themselves lack caller-owned tuning inputs.


### Stage-B VR code image no longer executes or loads

The direct-inner proof has now crossed the VR code-image boundary completely. First, the two residual callback pointers installed in the raw state (`+0xB0` history-transform callback and `+0xB8` transform callback) were set to null; all seven fixed profiles and both long live-switch tests remain exact, proving the source-owned history/late pipeline no longer dereferences either reference callback. All reference fallback-function bindings, mapped vtable writes, BL callsite patches and the reference deinitializer were then removed from the private adapter. The mapped `.text` plus `RT_CODE` pages were set `PROT_NONE`; seven-profile, deterministic-switch, randomized-switch and fresh-ASLR tests all remained bit-exact with unchanged hot-call topology.

The original small no-DLL checkpoint in this section was later found to share the same zero-PCM acceptance flaw as the compact cold-state proof; its `6ea2c4cde4cbfb83` hash is the zero-output regression hash and is **not correctness evidence**. The executable-dependency conclusion has since been re-established independently by the corrected v3 nonzero checkpoint described below: that adapter loads no `DolbyAPOVR.dll`, contains no PE-loader boundary, and remains bit-exact to the last trustworthy nonzero full-snapshot reference.


### Stage-B recovered payload fully externalized

The historical fixed-layout **v2** pack (46,388 bytes, SHA-256 `327803e94ab1072d2e5fcbc60974779632dbe8568ae79e54ee5587a720804b27`) is **superseded and invalid for correctness testing**. Rechecking it against the restored nonzero reference exposed three integration defects: the native `0x56B80` wrapper failed to publish the live late-row descriptor count, native `0x5CE70` omitted the deployed `0x5AD38` planar-to-interleaved stereo export, and v2 reversed the two 22-float history coefficient vectors while omitting live upper-analysis variation/projection data and late response/history-kernel data.

The corrected owner-supplied **v3** pack restores a conservative full cold snapshot and adds the missing live data. It is **344,397 bytes total** (96-byte header + 344,301-byte payload), SHA-256 `c993c123f2cb3b92776754da2383217e00b5f290664571f12cfb62b9afb3a175`. The proof source contains mutable semantic buffers but no recovered payload initializer. Missing or malformed v3 data is a hard initialization failure; the pack remains outside Git.

### Stage-B proof adapter no longer contains a PE-loader boundary

The private pack-only integration has now been reduced past the Windows-loader scaffolding itself. A new lean proof removes the PE-loader header/probe include, both `Sp11PeImage` members, all VR/VLLDP DLL path handling, loader/unloader calls, mapped-vtable/callsite patch helpers, dormant reference fallback pointers, VLLDP allocation/scheduler fields, and the obsolete reference callback trampolines. Proven-zero fallback branches now fail closed with `abort()` rather than retaining a vendor-code escape hatch. The resulting source contains no `sp11_pe_*` references and the resulting shared object contains no PE-loader/probe symbols or vendor-DLL path strings; its only dynamic dependencies are libc and libm.

The loader-removal architecture remains valid, but its original zero-output equivalence result is superseded. The corrected v3 lean proof reuses the loader-free/fail-closed structure and validates it against nonzero audio: the shared object has only libc/libm dependencies, contains no `sp11_pe_*` or vendor-DLL path strings, and `strace` with historical VR/VLLDP paths forced nonexistent records no vendor-DLL open. Explicit user-supplied Custom/GEQ target programming remains outside this proven built-in-profile contract.

### Corrected Stage-B v3 nonzero loader-free checkpoint

The Stage-B de-blob proof has been rebuilt from the last trustworthy nonzero full-snapshot reference rather than from the invalid zero-output chain. The corrected adapter includes the two missing live wrapper side effects, uses the correctly ordered history coefficient vectors, supplies the upper scheduler's variation/projection data and the late controller's response/history kernel, and loads the complete cold snapshot plus caller-owned tuning/model data from the external v3 pack. The archived proof source SHA-256 is `419f889d57c40a7db4de7df094d437a95ce2cabe86b3c2ac571f832d803064f0`.

Against the trustworthy nonzero reference, **all seven 200,000-frame fixed-profile runs have 0 differing float values** and output SHA-256 `cdfabfdbef8f7ed736d09543e4689a47fa884e0b4b53e76788d0adc3bdcdc3ec`. The fixed-profile Stage-B cadence is `376=781`, control/controller updates `3124`, scheduler upper `195`, lower-A `39`, lower-B `39`, and `56B80=781`, with zero relevant fallbacks. The deterministic 16-second live-profile differential is **0 / 1,536,000**, the randomized 30-second 33-switch stress is **0 / 2,880,000**, and host chunk schedules `64`, `480`, `1024`, `127/353`, and mixed all remain exact with regression hash `5006a28ef84ef6fb`. Five fresh-process/ASLR 20,000-frame runs all produce SHA-256 `243a76c275c41306ddcad11ce097f90bf55e0022827d6883d9385a65bf389d4c`.

The v3 adapter fails closed if its pack is missing. A traced standalone run with both historical vendor paths pointed to nonexistent files opens only the v3 data pack and makes **zero DolbyAPOVR/DolbyAPOvlldp150 open attempts**. This restores a valid no-DLL/PE-loader-free Stage-B integration checkpoint. Cold-state minimization is intentionally reset to the full 298,937-byte external snapshot until a new reduction is proven against this nonzero oracle; the old 34 x 64-byte seed and zero hash `6ea2c4cde4cbfb83` are retired.

### Stage-A endpoint postgain is native and reference-exact

The remaining production-relevant VLLDP endpoint-volume control has now been localized to an already-native semantic input instead of a new gain stage. Original `FUN_18001D280` converts the raw DAX postgain integer with the exact float32 sequence `SCVTF -> *2^-15 -> *0x1.f81f82p-1 -> *16` and stores it at VLLDP state `+0x65C`. At the live `FUN_180021E80` compressor callsite (`0x180020458`), `x4` is exactly `state+0x658`, so that scalar is `runtime[1]` of the five-float runtime vector already passed into `ubig_stage_a_compressor_process()`. No extra output multiplier is involved.
