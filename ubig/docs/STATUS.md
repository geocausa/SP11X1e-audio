...
This target is deliberately not installed by `all` and does not alter the Golden PipeWire graph. A tracked `deploy/ubig-candidate` wrapper now supplies the disposable promotion plumbing: `prepare.sh` rebuilds/tests/audits and stages the candidate without touching the active graph; `switch.sh activate` snapshots the exact active Golden PipeWire fragment, preserves the established node names and endpoint-volume ordering, opts the volume/MSIIR services into `ubig-control-v2`, and restores `MemoryDenyWriteExecute=yes`; `switch.sh rollback` restores the saved fragment and removes only candidate-specific drop-ins/control state. An isolated fake-systemd lifecycle test performed prepare → activate → rollback and recovered the original Golden fragment byte-for-byte. The remaining work is the M6 on-device lifecycle, xrun/NaN, long-run and acoustic gates.
