...
The candidate now consumes the public `ubig-control-v1` mmap ABI rather than the historical two-byte profile-control shim. A tracked private-pack gate (`candidate-control-check`) validates request/ack generation, in-place Dynamic→Movie→Custom switching, initial non-flat Custom target application, and a second **same-profile Custom EQ update** without reconstruction; the two otherwise-identical instances diverge on 640 / 960 output values only after the different second EQ request. With runtime control disabled for the differential, the tracked candidate remains bit-exact against the corrected-v3 private proof for all seven 200,000-frame profiles, a fixed non-flat Custom render, deterministic live switching (**0 / 1,536,000 differences**) and the randomized 30-second / 33-switch stress (**0 / 2,880,000 differences**).

This target is deliberately not installed by `all` and does not alter the Golden PipeWire graph. Its remaining promotion work is packaging/installation with explicit rollback plus the M6 on-device lifecycle, xrun/NaN, long-run and acoustic gates.
