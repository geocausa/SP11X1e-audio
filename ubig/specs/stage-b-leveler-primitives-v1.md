...
## SP11 20-band adaptive filter/update block
...
## Deployed Leveler control wrapper

`ubig_stage_b_leveler_wrapper_process()` closes the thin deployed control layer immediately above the native stereo Leveler parent. The public contract contains only observed SP11 behavior: target-limit slew, force/direct one-shot state, optional target/lookup scalar overrides, the Voice-style disabled path, and final two-scalar redistribution. The reference's alternate generated-control branch is never entered by any shipped profile and is intentionally absent. Its curve records therefore remain caller-owned inputs rather than embedded reference data.

The original wrapper has one register-lifetime quirk that is relevant only to the private interoperability oracle: a live sixth integer register is forwarded as transient scratch even though a decompiler omits it from the apparent prototype. UbiG's semantic API has no such ABI dependency. A wrapper-level snapshot/reference/restore/UbiG replay gate leaves the reference child unpatched and compares the full persistent VR arena (excluding the child's already-classified transient scratch bank), both row descriptors, telemetry, and both scalar outputs. Promoted-source result: **2,688/2,688 complete warm replays bit-exact** across all seven profiles (64 calls x six chunk-pattern instances per profile), with zero wrapper or child fallback. Public all-synthetic lifecycle hash: `c03c38ccf02019e9`.
