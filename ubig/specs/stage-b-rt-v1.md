...
The deployed 48-kHz stereo integration does not require the proprietary `0x18002BE18` constructor body. The validated integration boundary is zeroed persistent storage plus **34 surviving 64-byte regions (2,176 bytes total)** reconstructed with allocation-relative fixups and explicit caller-owned data bindings. This is the smallest currently accepted boundary that preserves not only bit-exact audio but also the observed Stage-B execution topology and scheduler cadence across fixed-profile and live-switch tests.

Audio equality alone is not sufficient evidence for further constructor-state reduction: a smaller candidate is rejected if it changes the hot native call graph or cadence even when current output samples remain equal. A valid source integration may therefore build the cold state semantically from the validated relocatable region set rather than preserve or execute the original constructor. The proprietary raw layout remains an implementation detail and is not part of the public UbiG ABI; further reduction requires the same topology-preserving guard.
...
This loader-free adapter is bit-exact against the preceding no-DLL pack-only checkpoint for all seven built-in profiles and both deterministic and randomized live-switch tests. Its binary links only the ordinary C/math runtime, and tracing with historical VR/VLLDP DLL paths pointed at nonexistent files records no vendor-DLL open attempt. Explicit Custom/GEQ target programming is still a separate cold-control feature and is not included in this boundary; built-in profile operation does not require it.
