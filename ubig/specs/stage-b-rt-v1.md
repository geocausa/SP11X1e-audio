...
The deployed 48-kHz stereo integration does not require execution of proprietary constructor `0x18002BE18`. However, the former **34 x 64-byte / 2,176-byte** compact cold-state claim is invalidated: its integration oracle had accidentally collapsed to all-zero PCM, so topology matching was insufficient to establish audio equivalence. That seed is not an accepted semantic boundary.

The current conservative loader-free proof restores the complete **298,937-byte initialized cold snapshot as owner-supplied external data**, rebases allocation-relative references, and then replaces live image-relative data with semantic caller-owned bindings. Further cold-state reduction must compare against the corrected nonzero oracle and preserve relevant state/cadence as well as output. The proprietary raw layout remains a private integration detail and is not part of the public UbiG ABI.


## Cold-core caller-owned data boundary
...
A blob-free executable integration may keep recovered endpoint-specific data outside the program image. The corrected private **v3** checkpoint uses an owner-supplied pack containing the full conservative cold snapshot plus caller-owned history, band, deep-controller, control-model, projection/variation and late-controller data needed by the shipped 48-kHz stereo route. The adapter loads those bytes into mutable semantic buffers before construction; the executable contains no recovered Stage-B payload initializer and does not load the reference VR DLL.

The historical v2 compact pack is superseded because its zero-PCM proof hid missing live data and reversed the two history coefficient vectors. Missing or structurally incompatible v3 data is a hard initialization failure; there is no reference-code fallback. Public algorithms and ABI remain independent of this private pack format, which is an integration proof rather than a stable UbiG public file format.

## Loader-free private integration boundary

The validated private Stage-B endpoint adapter no longer needs any Windows PE-loader API or reference-image lifecycle object. Once the external Stage-B data pack has populated the caller-owned buffers, cold construction and realtime processing are entirely source-owned: no `Sp11PeImage`, DLL path, mapped vtable, callsite patch, reference callback trampoline, deinitializer, or vendor-code fallback is part of the deployed 48-kHz stereo contract. A failed semantic contract check fails closed instead of dispatching back into a reference image.

The corrected v3 loader-free adapter is bit-exact against the last trustworthy **nonzero** full-snapshot reference for all seven built-in profiles and both deterministic and randomized live-switch tests. Seven 200,000-frame fixed-profile runs have zero differing float values; deterministic switching is 0 / 1,536,000 differences and randomized 33-switch stress is 0 / 2,880,000. Its binary links only the ordinary C/math runtime, and tracing with historical VR/VLLDP DLL paths pointed at nonexistent files records no vendor-DLL open attempt. Explicit Custom/GEQ target programming remains a separate cold-control feature and is not included in this boundary.
