...
The candidate uses the public `ubig-control-v2` page at `$XDG_RUNTIME_DIR/ubig-control-v2`. Candidate drop-ins set `UBIG_CONTROL_FORMAT=ubig-v2` for the established endpoint-volume and MSIIR services, while the installed Golden bridge keeps its legacy control page when the candidate drop-ins are absent. The filter-chain candidate drop-in also restores `MemoryDenyWriteExecute=yes`; the old `MemoryDenyWriteExecute=no` exception remains on disk solely for rollback and becomes effective again when the candidate drop-in is removed.
