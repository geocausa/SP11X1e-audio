...
sudo -u "$TARGET_USER" HOME="$TARGET_HOME" "$SCRIPT_DIR/verify-v31-candidate.sh"
ROOT_DEV=$(findmnt -n -o SOURCE /)
ROOT_UUID=$(blkid -s UUID -o value "$ROOT_DEV")
[[ -n "$ROOT_UUID" ]] || { echo "could not resolve root UUID" >&2; exit 2; }
OUT=/etc/grub.d/99_sp11_audio_golden_v31_ckv_delta_candidate
D=/boot/sp11-7.1.5-audio-golden-v31-ckv-delta
cat >"$OUT" <<GRUB
#!/bin/sh
exec tail -n +3 \$0
menuentry 'SP11 Audio Golden v31 — prior/new CKV CANDIDATE' --id 'sp11-audio-golden-v31-ckv-delta-candidate' --class ubuntu --class gnu-linux --class gnu --class os {
 load_video
 set gfxpayload=keep
 insmod gzio
 insmod part_gpt
