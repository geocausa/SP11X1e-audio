...
Q6DISK=/lib/modules/$KREL/kernel/sound/soc/qcom/qdsp6/snd-q6apm.ko.zst
check() {
...
check acf8ba1e6ded43cfac86afa7d12dd30dabd972286e74b021fbb7bddf98955033 "$Q6DISK"
[[ "$(modinfo -F srcversion "$Q6DISK")" == 687B16CF9C43B43E90C0746 ]]
echo "OK disk q6apm srcversion=687B16CF9C43B43E90C0746"
check_live() {
...
if grep -qw 'sp11_entry=7.1.5-sp11-golden-v31-ckv-delta' /proc/cmdline; then
  ...
  mute_numid=$(sed -n "s/^numid=\([0-9]*\).*name='SP11 Windows Endpoint Mute'.*/\1/p" <<<"$controls")
  ...
  [[ $(amixer -D hw:0 cget numid="$mute_numid" | sed -n 's/.*type=BYTES.*values=\([0-9]*\).*/\1/p') == 12 ]]
  echo "OK v31 fixed controls present with 288/16/12-byte capacities"
fi
