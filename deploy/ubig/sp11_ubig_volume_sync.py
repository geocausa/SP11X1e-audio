#!/usr/bin/env python3
"""Reproduce the SP11 Windows endpoint taper and feed its dB state to UbiG.
...
helper therefore treats the virtual UbiG sink as the user-facing scalar,
...
The virtual UbiG sink retains the user's visible slider value. Only the hidden
downstream hardware sink is re-tapered, so endpoint attenuation remains after
the reference Windows processing position. The separate MSIIR service
...
DEFAULT_NODE = "effect_input.sp11_ubig"
...
def apply_state(state: tuple[float, bool], hardware_id: int | None, control: Path,
    ...
    if not dry_run:
        ...
        # Endpoint attenuation is downstream of the reference processing path on Windows.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"sp11-ubig-volume-sync: {exc}", file=sys.stderr)
        return 2
