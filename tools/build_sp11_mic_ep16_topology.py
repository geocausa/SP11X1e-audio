...
By default this emits a *structural-only* topology and deliberately omits the
PCM_CNV interleave token because the EP16 capture payload for PID 0x08001008
has not yet been observed.  Pass --pcm-interleave only after that value is
closed from Windows evidence.
"""
...
def module_tuple(module, sgprop, contprop, outgoing, pcm_interleave):
    ...
    if iid == IID_PCM_CNV and pcm_interleave is not None:
        mod_lines.append(f"token252 {pcm_interleave}")
...
def render(modules, by_iid, sgmeta, containers, edges, pcm_interleave):
    ...
    lines = [
        ...
        "# PCM_CNV interleave is intentionally omitted unless --pcm-interleave is supplied.",
        "", "SectionWidget {",
    ]
    for m in modules:
        iid = integer(m["iid"])
    ...
    for m in modules:
        lines.append(module_tuple(m, sgmeta[integer(m['subgraph_id'])], containers[integer(m['container_id'])], outgoing[integer(m['iid'])], pcm_interleave))
    lines += ["}", "", "SectionData {"]
    for m in modules:
...
def main():
    ...
    ap.add_argument("--pcm-interleave", type=int, choices=(1,2,3), default=None,
                    help="Windows-confirmed PCM_CNV interleave; omit until evidence is closed")
    args=ap.parse_args()
    data, modules, by_iid, sgmeta, containers, edges=load_oracle(args.oracle)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(render(modules,by_iid,sgmeta,containers,edges,args.pcm_interleave))
    print(json.dumps({
        "output": str(args.output), "graph_id": GRAPH_ID, "modules": len(modules),
        "edges": len(edges), "subgraphs": [hx(x) for x in sorted(sgmeta)],
        "backend_id": BACKEND_ID, "pcm_interleave": args.pcm_interleave,
        "runtime_complete": args.pcm_interleave is not None,
    }, indent=2))
