#!/usr/bin/env python3
"""Build the reviewed Windows DEFAULT speaker structural contract."""
...
EXPECTED_PACKET_SHA256 = (
    "001e8f78c2aed10e74faad7c2f70095f307a52d4590ec0d00e785e57f44a006e"
)
EXPECTED_SUBGRAPHS = {"0xb0000001", "0xb000007e", "0xb000007f"}
...
def _link_identity(link: dict[str, Any]) -> tuple[str, ...]:
    ...
    return (
        link["peer_1_iid"],
        link["peer_1_control_port"],
        link["peer_2_iid"],
        link["peer_2_control_port"],
        intents[0],
        heaps[0],
    )
...
def _classify_connection(edge: dict[str, Any]) -> dict[str, Any]:
    ...
    if edge["scope"] != "external_destination":
        ...
        result["peer_identity"] = (
            "speaker-loopback SAL in subgraph 0xb0000045"
        )
    else:
...
def _classify_control_link(link: dict[str, Any]) -> dict[str, Any]:
    ...
    if LOOPBACK_TIMER_PEER in peer_iids:
        ...
        result["peer_identity"] = (
            "RATE_ADAPTED_TIMER in subgraph 0xb0000045"
        )
    else:
        result["baseline_disposition"] = "admitted"
    return result
...
def build_model(graph: dict[str, Any], input_sha256: str) -> dict[str, Any]:
    """Validate the reviewed graph and return the compact structural model."""
    ...
    if set(graph.get("subgraph_ids", [])) != EXPECTED_SUBGRAPHS:
        raise ValueError("unexpected DEFAULT subgraph set")
    ...
    for group in groups:
        for module in sorted(
            group["modules"], key=lambda item: _iid_number(item["iid"])
        ):
            iid = module["iid"]
            if iid in seen_iids:
    ...
    connections = [
        _classify_connection(edge) for edge in graph["resolved_connections"]
    ]
    connections.sort(
        key=lambda edge: (
            _iid_number(edge["source_iid"]),
            edge["source_port"],
            _iid_number(edge["destination_iid"]),
    ...
    control_links = [
        _classify_control_link(link)
        for link in graph["resolved_control_links"]
    ]
    ...
    return {
        "format": "sp11-windows-default-speaker-structural-model",
        "format_version": 1,
        ...
        "linux_translation_gates": {
            ...
            "dolby": "userspace identity/bypass boundary only",
        },
    }
...
def main() -> int:
    ...
    model = build_model(graph, hashlib.sha256(raw).hexdigest())
