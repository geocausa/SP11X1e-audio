...
int main(int ac,char**av){
 ...
 else if(!strncmp(mode,"chunk:",6)){
   unsigned long long off=0,len=0;
   if(sscanf(mode+6,"%llx:%llx",&off,&len)==2 && off+len<=VR_OUTER_SIZE) memcpy(outer+off,cap+off,(size_t)len);
 }
 else if(!strncmp(mode,"pair:",5)){
   unsigned long long o1=0,l1=0,o2=0,l2=0;
   if(sscanf(mode+5,"%llx:%llx:%llx:%llx",&o1,&l1,&o2,&l2)==4 && o1+l1<=VR_OUTER_SIZE && o2+l2<=VR_OUTER_SIZE){
      memcpy(outer+o1,cap+o1,(size_t)l1); memcpy(outer+o2,cap+o2,(size_t)l2);
   }
 }
 free(cap);
 ...
 for(unsigned k=0;k<1024;k++){memset(out,0,sizeof(out));tone_at(in,N,(uint64_t)k*N);proc(inner,out,in,N);if(k==1023){float pk=0;for(unsigned j=0;j<N*2;j++)if(fabsf(out[j])>pk)pk=fabsf(out[j]);printf("block=%u fill=%u rms=%.9f peak=%.9f\n",k,d(inner,0x20),rr(out,N*2),pk);}}
 return 0;
}
