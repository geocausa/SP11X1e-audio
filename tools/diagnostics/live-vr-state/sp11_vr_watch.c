...
int main(int ac,char**av){
 ...
 if(getenv("SP11_DIAG_STOP")) raise(SIGSTOP);
 ProcessFn proc=(ProcessFn)sp11_pe_ptr_for_va(&p.vr_img,VR_PROCESS_VA);enum{N=256};float in[N*2],out[N*2];
 for(unsigned k=0;k<4;k++){memset(out,0,sizeof(out));tone_at(in,N,(uint64_t)k*N);proc(inner,out,in,N);if(k==31||k==255||k==1023||k==4095){float pk=0;for(unsigned j=0;j<N*2;j++)if(fabsf(out[j])>pk)pk=fabsf(out[j]);printf("block=%u fill=%u rms=%.9f peak=%.9f\n",k,d(inner,0x20),rr(out,N*2),pk);}}
 return 0;
}
