...
> **2026-08-08 state-pinned continuation:** the Aug-7 Dynamic Windows capture
> closes the old "normal limiter remains ~2.47 dB below threshold" boundary for
> the real Windows graph. With `peak-level=0`, `system-gain=0` and negative
> postgain, the live final limiter is attenuating and the inner VLLDP input is
> already near full scale from a `-3 dBFS` source step. The limiter mechanism in
> this document remains correct; the remaining-gap explanation is superseded by
> `2026-08-07-STATE-PINNED-DYNAMIC-VLLDP-PRELIMITER-DRIVE.md`.
