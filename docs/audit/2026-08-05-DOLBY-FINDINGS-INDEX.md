...
| `2026-08-06-MAY18-NONLINEAR-RESIDUAL-LOCALIZATION.md` | **Current residual localization**: realistic VLLDP endpoint postgain closes most of the seven-step amplitude curve; AudioEng is only the final 0.985 ceiling; `vlldp-peak-level` selectively activates a May-like odd-saturation branch but remains 0 in shipped/live evidence, so it is a mechanism probe rather than a production setting. |
