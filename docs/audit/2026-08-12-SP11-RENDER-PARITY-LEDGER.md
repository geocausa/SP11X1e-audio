...
- [~] **AMBER L04 — Windows SOFT_PAUSE identities are DSP-confirmed live; Linux trigger ordering failed and correction `0051` awaits reboot.** Windows DEFAULT uses iid `0x466b`. Driver disassembly recovers tag `0x04010008`, key `0x01000021`, PAUSE/state-3 -> value `1`, RUN/release/state-4 -> value `0`; STOP also releases any outstanding pause. REV_0D maps value `1` to zero-length pid `0x0800102e` and value `0` to `0x0800102f`; completion events are `0x0800103f`/`0x08001043`. A zero-valued direct ALSA pause on `7.1.5-sp11-render-parity+` timed out, then both correct events arrived after the trigger stall ended. Patch `0051` moves the host to STOPPED/state 3 before sending PAUSE so serialized pull-watermark callbacks cannot block completion behind ALSA's held PCM stream lock. Reverse applicability, strict checkpatch, focused tests and exact-tree `W=1` object compile pass; corrected pause/resume/STOP/reprepare evidence after a new boot still gates GREEN.
