...
- [~] **AMBER H03 — WSA DRE/CSR fallback register lifecycle.** Fresh Windows KDNET proves both amplifiers initialize `DRE_CTL_1 0x34b1=0x00` and perform zero runtime writes to it across two complete PA start/stop cycles, while pre-candidate Linux is `0x0f` because UCM writes PA Volume 24 and generic unmute sets `CSR_GAIN_EN`. Patch `0055` plus the matching UCM correction are built as an isolated signed candidate; live validation is pending. See `docs/findings/2026-08-15-WINDOWS-WSA-DRE-CTL1-LIFECYCLE-GAP.md`.
