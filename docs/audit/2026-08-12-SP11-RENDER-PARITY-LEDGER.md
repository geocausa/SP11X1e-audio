...
- [~] **AMBER V04 — final endpoint-gain actuator is live and Windows lifecycle-proven; per-channel sequencing/physical validation remain.** Fresh Windows KDNET directly captures qcadcm SetVolume sending final `0x4a63/0x08001038` and then GetGraphCkv on live 8%↔17% gestures, while VLLDP remains frozen. Linux v4 now reproduces that split lifecycle and sends final VOL_CTRL before the complete GainStep group with fail-quiet host handover. The remaining structural difference is narrower: Windows master volume is emitted as two channel-ordered SetVolume calls (intermediate mixed L/R body), whereas Linux currently emits one simultaneous stereo body. Physical slider/seek listening on the corrected topology still gates overall GREEN. See `docs/findings/2026-08-14-WINDOWS-LIVE-VOLUME-LIFECYCLE-KDNET.md`.
...
- [~] **AMBER L03a — live volume lifecycle is now Windows-causal; one channel-order difference remains.** Fresh KDNET removes the old MSIIR-listening ambiguity: Windows itself runs qcadcm SetVolume→final VOL_CTRL→GetGraphCkv/dependent MSIIR on each live endpoint update, while neither recovered VLLDP postgain setter fires. Linux now follows that lifecycle and a real 12%→8%→17%→8%→12% run kept VLLDP frozen while switching Q28/GainStep with zero WSA/transport faults. Windows, however, performs two channel-ordered SetVolume calls for one stereo master gesture; Linux still emits one simultaneous stereo body. That concrete sequencing difference, plus fresh physical listening, is the next gate.
...
1. **W03 — explain the remaining physical tonal mismatch:** H07 channel identity is now closed and the Windows-width protection path is live. Corrected DP5 `0x03` is loaded on both amplifiers; VI/CPS, SP/SPVI, graph startup, bounded playback and suspend gates pass; the retained Windows hardware transaction is transport/profile closed; and DP4 remains correctly unscheduled. Continue matched downstream-state investigation without inventing an unobserved processing module.
