...
1. **H03 — isolate SP11 SoundWire topology-runtime retention across PA-off idle:** live qcaucd WPP now proves Surface `SwrClockStopTimerMS=500` is operational: both controlled Windows cycles enter the delayed SoundWire clock-stop sequence about 0.485–0.499 s after stream close. Windows also replays dataport programming on every start/stop, so active-port retention is not the answer. The concrete Linux-only difference is object lifetime: WSA `hw_free()` calls `sdw_stream_remove_slave()` (freeing slave/port runtimes) and qcom `shutdown()` releases the whole stream runtime; the next wake allocates/rebuilds it. The next candidate must keep PA/producer off and still disable/deprepare/clock-stop the bus, but retain/reuse the SP11 WSA playback/VI/CPS topology objects. See `docs/findings/2026-08-17-WINDOWS-QCAUCD-SWR-CLOCKSTOP-LIFECYCLE.md`.
