...
1. **H03 — reproduce a coherent, address-audited Windows WSA8845 board/supply init block:** v9-v11 close isolated DRE-history variables; provenance-clean v12R proves that moving only final `CURRENT_LIMIT=0x44` earlier is cycle-unstable and must not be optimized around. Build the Linux-vs-Windows ordered transaction map from raw addresses, then replay the smallest coherent manufacturer block (not mislabeled symbols or isolated values) on SP11 2S only. First gate is multiple consecutive 1% muted digital-zero PA cycles on one boot; no music/chirp until the floor is both near Windows and repeatable.
