...
- [ ] **AMBER H03 — WSA DRE/CSR consumer semantics are narrowed to write timing/history; route-time-zero v7 has passed its first safety gates but acoustic closure is still pending.** Windows initializes both WSA8845s with `DRE_CTL_1=0x00` and performs zero ordinary runtime writes to that register. v5 proved that keeping `CSR_GAIN_EN` clear under the corrected passive-idle host lifecycle is bounded-safe and directionally improves the matched no-HD2 predecessor (five-run 1--5 kHz MAE about 0.527 -> 0.432 dB). v6 is permanently rejected: it forced `CSR_GAIN[5:1]=0` at PA unmute and produced wildly non-repeatable 12% response despite clean low-level lifecycle. Read-only regmap tracing explains the distinction: Linux normally writes the gain field during UCM/profile construction before SoundWire resume, while ordinary demand playback only touches the enable bit. v7 therefore keeps the exact v5 kernel/unmute path and changes PA Volume 24 -> 31 only in a boot-scoped read-only UCM overlay, placing raw gain code zero at the natural route/profile boundary. Cold boot verified the exact v5 module, read-only overlay and PA controls 31; the route trace shows `mask=0x3e val=0` before SoundWire and ordinary demand shows only `mask=0x01 val=0`. A real 1% fixed-MP3 cycle reached physical PCM RUNNING and returned cleanly to `closed` with no new WSA/PA/SoundWire/XRUN/ADSP fault, and the subsequent idle watcher stayed `closed` for all 90/90 samples. v7 is **not yet GREEN**: 5% program audio and the byte-identical 12% five-run acoustic oracle remain to be executed before any promotion. See `docs/findings/2026-08-16-WSA-CSR-ROUTE-ZERO-V7-CHECKPOINT.md`.
