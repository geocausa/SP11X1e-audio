...
- [ ] **RED H03 — WSA DRE/CSR fallback lifecycle is not yet Windows-equivalent.** Fresh Windows KDNET proves both amplifiers initialize `DRE_CTL_1 0x34b1=0x00` and perform zero runtime writes to it across ordinary PA cycles, while the native Linux unmute path later forces `CSR_GAIN_EN=1`. The isolated Linux DRE-disabled cold-boot candidate produced ugly/unsafe speaker noise and is permanently rejected. The exact loaded CPS-v3 binary is now traced through its full active producer transition: SoundWire is enabled first; both WSA interpolators/SmartBoost/COMP paths are configured; `COMPANDER0/1_CTL0` are activated, RX0/RX1 COMP enable is asserted, and the half-dB PGA state is enabled; final macro POST_PMU completes **29.769 ms before first WSA8845 unmute**. This closes producer timing and proves the actual Linux runtime state without relying on a drifted reconstructed source. Together with the first-MCLK and Windows qcadcm/DP2 closures, the remaining gap is specifically producer gain/COMP semantics and their contract with WSA8845 DRE. H02 now separately tracks the Linux-only -3 dB macro safety cap; the active half-dB/gain-offset policy is also not yet Windows-proven. Production remains on known-good CPS-v3 CSR-assisted mode. See `docs/findings/2026-08-15-WSA-ACTIVE-RUNTIME-PRODUCER-STATE.md`.
