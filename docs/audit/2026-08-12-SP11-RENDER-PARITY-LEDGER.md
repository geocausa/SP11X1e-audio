...
- [ ] **AMBER H03 — WSA DRE/CSR consumer semantics remain open; v5 is the bounded-safe reference, v7 and v8 are valid static/noise rejections, and old v6 causal attribution is retracted.** Windows initializes both WSA8845s with `DRE_CTL_1=0x00` and performs zero ordinary runtime writes to that register. v5 keeps `CSR_GAIN_EN` clear under the corrected passive-idle host lifecycle, leaves stored gain code 7, is bounded-safe, and improves the matched no-HD2 1--5 kHz median from about 0.527 to 0.432 dB MAE. The old packaged v6 module is provenance-invalid: its `wsa884x_mute_stream()` machine code is identical to v5 despite the candidate source containing an extra gain-zero call, so the old v6 acoustic instability cannot be attributed to that intended source delta. v7 remains a valid experiment because it deliberately reused proven v5 module bytes and changed only UCM PA Volume 24 -> 31 at route time; it produced operator-observed active-playback static/noise on the first exact 12% chirp and is rejected. Provenance-clean v8 kept v5's UCM/value state and changed only ordinary SP11 2S DRE write history. A read-only trace proved the desired sequence: COMP-aware POST_PMU clears `DRE_CTL_1` once per amp, then ordinary PA start is exactly `CLSH67 -> PWR08 -> PDRV52 -> PA=1 -> PWR0c -> PDRV5a` with **zero DRE writes**, and stop is `PA=0 -> CLSH=0` with zero DRE writes. v8 nevertheless fails an even stronger discriminator: a 10-second 48 kHz stereo S16 all-zero WAV played at visible endpoint 1% while muted held PCM RUNNING and raised SP7 external-mic RMS from about 0.000102/0.000105 baseline to 0.001278/0.001512 on channels 0/1 (about 12.6x/14.3x), with the elevated broadband noise persisting while PA remained open after the zero file ended. The 90-second idle gate itself stayed 90/90 closed. v8 is therefore rejected without 5% or 12% escalation. Matching final DRE value or ordinary PA write history alone is insufficient; pursue an earlier Windows WSA8845 initialization/state dependency, using full write-sequence comparison rather than value sweeping. See `docs/findings/2026-08-16-WSA-V6-PROVENANCE-RETRACTION-AND-DRE-COLD-V8.md` and `artifacts/reviewed/2026-08-16-v8-zero-stream-static-rejection.json`.
...
1. **H03 — compare the full Windows and Linux WSA8845 initialization/write sequence before another candidate:** v8 proves that removing the extra ordinary PA-boundary DRE writes is not sufficient; the resulting active digital-zero state is noisy despite exact Windows-like PA start/stop ordering. Keep v5 as the bounded-safe reference. Do not re-arm v7/v8 or sweep DRE values. Capture/compare the complete amp write history around route construction, COMP setup and first PA activation to find the earlier state Windows establishes before its no-DRE-write runtime cycles.
