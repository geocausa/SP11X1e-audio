...
- [ ] **RED L03 — real YouTube seek path localized through both OSes; downstream Qualcomm parity candidate live.** Genuine Firefox/YouTube Linux captures around verified seeks show the transient already at `effect_output.sp11_windows_dolby`; the aligned ALSA hardware-sink monitor is a sample-for-sample copy (scale 1.000000, correlation 1.000000, zero residual). The all-zero APO `SILENT`-flag hypothesis was rejected offline and not deployed. A matched Windows real-YouTube run at the same 25% / -20.74741 dB endpoint state also reaches roughly full-scale (~0.987) in WASAPI loopback after seeks even though the physical Windows speakers sound smoothly capped. This localizes the missing physical smoothing below the Windows loopback / Linux PipeWire monitor boundary. Audit of the reviewed Windows DEFAULT graph then found Linux had dropped the fourth module-control link `POPLESS_EQ 0x4664 <-> VOL_CTRL 0x4663`, intent `INTENT_ID_P_EQ_VOL_HEADROOM (0x08001118)`. The generator is fixed and an isolated four-link candidate has booted successfully with the entire SP/SPVI/CPS chain and `GRAPH_START` accepted. Immediate gate: physical/WSA-side YouTube seek A/B on this candidate before promotion.
- [~] **AMBER/RED L04 — Windows SOFT_PAUSE stream lifecycle recovered; generated SET calibration still missing.** Windows DEFAULT uses iid `0x466b`. Driver disassembly recovers the exact control state machine: tag `0x04010008`, key `0x01000021`, PAUSE/state-3 -> value `1`, RUN/release/state-4 -> value `0`; STOP also releases any outstanding pause. PAUSE_COMPLETE `0x0800103f` and RESUME_COMPLETE `0x08001043` are registered separately. The earlier zero-filled `0x0800102d`/`0x0800103e` KD buffers were captured in `gsl_get_tagged_custom_config()` and are GET/readback request buffers, not proven SET payloads. Linux already instantiates SOFT_PAUSE and registers its events, but q6apm drops non-watermark module events and q6apm-dai does not drive this lifecycle. Next gate: recover the actual calibration SET(s) generated by `tag 0x04010008 + TKV 0x01000021={0,1}` before implementing; do not inject guessed zero payloads.
...
3. **L04 — implement Windows SOFT_PAUSE lifecycle:** the Windows state machine/TKV is recovered (`tag 0x04010008`, key `0x01000021`, pause=1/release=0), but the calibration SET(s) generated by that TKV still need exact recovery. The earlier zero-filled `0x466b` buffers were GET/readback requests, not commands. Recover the generated SET path, then reproduce pause/release + completion-event ordering without conflating it with in-stream seek.
