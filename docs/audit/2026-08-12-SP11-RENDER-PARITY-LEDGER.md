...
> **2026-08-18 v31 CKV-delta / acoustic-method update:** the isolated
> `agent/golden-v31-ckv-delta-20260818` candidate corrects a later-discovered
> Windows semantic omitted by the earlier volume-transaction work. Qualcomm
> GSL/ACDB uses prior-CKV -> new-CKV changed-key delta semantics: final
> `VOL_CTRL` is still sent on each channel update, but the four-frame
> GainStep/MSIIR OOB calibration is sent only when the selected GainStep CKV
> actually changes. v31 adds a fixed 16-byte stereo final-volume-only control
> and live userspace emits same-row `vol->vol`, upward-boundary `cal->vol`, and
> downward-boundary `vol->cal`. Two independent fixed-geometry 40-Hz SP7
> captures reduce the v30 Volume-Up p95 from `2.7855e-3` to `6.6466e-5` and
> `6.4095e-5`, with UP/DOWN ratios `0.975x` and `0.945x`; native Windows was
> `6.1937e-5` and `1.006x`. Deterministic Seven Nation Army seeks also retain
> the v28 physical closure. v31 remains a one-shot candidate pending the
> operator listening verdict; saved default remains Golden v28.
>
> The SP7 acoustic calibration method was also tightened. Cross-capture
> absolute L/R/bass tuning must now use `AUDCLNT_STREAMOPTIONS_RAW`; the older
> shared-mode microphone-array captures are retained as historical evidence but
> their absolute cross-OS dB differences are provisional. Three Linux-v31 RAW
> passes repeat to about `0.09..0.13 dB` median range from 630 Hz to 6.3 kHz.
> See `docs/findings/2026-08-18-SP7-WASAPI-RAW-ACOUSTIC-CALIBRATION.md`.
...
**AMBER overall — Golden v31 is the operator-promoted saved daily-driver/default and retains v28 as rollback plus CPS-v3 as rescue. The current `agent/psycho-bass-20260818` userspace candidate closes the old Linux WSA RX81/-3 dB producer-policy gap with the directly Windows-proven RX84/0 dB state after protected producer wake. Its objective 40-Hz CKV, exact-DSP-mute, program-seek, idle/wake lifecycle and physical bass-transfer gates are GREEN. It is not yet merged into Golden/main: operator normal-listening/bass judgment remains the promotion gate. Remaining physical research is narrowed to pinned-gain SP7 WASAPI-RAW L/R/upper-bass residual characterization rather than a guessed missing bass effect. P09 and W02 remain explicitly non-blocking; clean public source packaging remains separate from sound-quality closure.**

## 1. Base Linux render path

- [x] **GREEN R01 — Kernel / ASoC platform stack.** Current live Golden v31 uses the accepted `7.1.5-sp11-render-parity-v4+` kernel/DTB with the promoted signed `snd-q6apm` initramfs override for endpoint-volume parity; X1E80100, q6apm/q6prm/APR, SoundWire and WSA884x are live. Saved/default boot is Golden v31; Golden v28 is rollback/comparison and CPS-v3 is rescue.
...
- [x] **GREEN H02 — LPASS WSA-macro digital gain is Windows-proven at 0 dB, including active-lifecycle validation.** Passive qcaucd KD tracing through the native Qualcomm register helper directly captured Windows `RX0/RX1 RX_VOL_CTL=0x00`, i.e. 0 dB. Golden v31's machine driver is RX84-capable but normal Linux policy still parks the controls at the older RX81/-3 dB safety value while idle. The current userspace candidate applies RX84 once **after** the protected producer is active, restores RX81 on idle, and does not rewrite the gain on ordinary volume/mute events. A continuous 100-Hz actuator test measures the expected +3.0..3.1 dB physical step; integrated service control reproduces manual RX84 within ~0.062 dB mean absolute error across the core bass probe. 40-Hz CKV stress, exact mute and deterministic seek gates remain GREEN. RX84 is therefore the correct Windows structural policy, not an acoustic guess; only the operator listening/promotion decision remains. See `docs/findings/2026-08-16-WINDOWS-WSA-NATIVE-PROGRAMMING.md`, `docs/findings/2026-08-18-PSYCHOACOUSTIC-BASS-WINDOWS-RX84-PRODUCER-GAIN.md`, `docs/findings/2026-08-18-PSYCHOACOUSTIC-BASS-RX84-LIFECYCLE-LIVE-GATE.md`, and `docs/findings/2026-08-18-RX84-40HZ-PROGRAM-MUTE-SEEK-COMPATIBILITY.md`.
...
- [x] **GREEN H09 — DP1/DP2/DP3 WSA8845 SIMPLE slave-transport fields match Windows on the isolated v30 candidate.** A live `sdw_write_no_pm()` trace captured both physical amplifiers and both register banks. DP1/DAC BlockCtrl3 is `0x00`, Golden-v28 DP2/COMP OffsetCtrl2 remains `0x07`, and DP3/BOOST OffsetCtrl2 is `0x1f`; each exact value repeated on both amps. DP1/DP3 are capability declarations only: the Qualcomm master schedule supplies the values, so no codec-side transport constant is invented. The same candidate retains the exact 10 START + 6 STOP lifecycle after a 20-second idle and remains at the v28/Windows physical noise floor. See the 2026-08-18 Golden-v30 live validation finding.
...
- [x] **GREEN Q10 — Windows live CKV re-selection on endpoint-volume changes is directly KDNET/static-GSL proven.** Fresh SP11 Windows KDNET resolves qcadcm SetVolume at RVA `0x6e038`: each live endpoint update sends final `0x4a63/0x08001038` and calls GetGraphCkv; the controlled 8%→17%→8% run selected CKV1/2/1 with the expected intermediate louder-channel CKV. Follow-up hash-bound GSL/ACDB reconstruction sharpens the runtime rule: `AcdbComputeDeltaCKV(old,new)` contains only changed keys, so the dependent `0x489e` GainStep calibration group is emitted only when the selected GainStep CKV changes. Linux v31 implements that prior/new delta semantic rather than blindly replaying the same row. See `docs/findings/2026-08-14-WINDOWS-LIVE-VOLUME-LIFECYCLE-KDNET.md` and `docs/findings/2026-08-18-GOLDEN-V31-CKV-DELTA-40HZ-PHYSICAL-GATE.md`.
...
- [x] **GREEN P10 — effective Windows HLOS CPS semantics are closed for this SP11 driver build.** Exhaustive hash-locked qcadcm SET_CFG/OOB searches did not find public `PARAM_ID_CPS_LPASS_HW_INTF_CFG (0x08001259)`; instead qcaucd locally classifies the WSA8845 identity family, writes selector `5`, and copies image-backed slots 13/14 into live SoundWire state. Every template field that produces a live DP6 write is mapped to its standard transport role: shared physical master port 13, slave DP6, mask `0x03` on both, 24 kHz / 800 clocks, SampleCtrl `0x1f/0x03`, left/right OffsetCtrl1 `0/25`, HCtrl `0xff`, BlockCtrl1 `0x18`, BlockCtrl3 `0x00`; `0xff` optional fields suppress writes and slot 14 is a right-slave-only companion rather than master port 14. CPS-v3 implements the same effective values through normal SoundWire/ASoC paths. This closes the HLOS parity requirement without inventing a public payload that the reviewed Windows stack does not send; P09 dynamic telemetry remains separate. See `docs/findings/2026-08-18-WINDOWS-CPS-HLOS-EFFECTIVE-SEMANTICS-CLOSEOUT.md`.
...
- [x] **GREEN V04 — final endpoint-gain actuator and channel ordering match Windows live.** Fresh Windows KDNET proves one stereo master gesture is delivered as left=new/right=old followed by left=new/right=new at final `0x4a63/0x08001038`. v31 preserves those exact mixed/final Q28 bodies while applying the recovered prior/new CKV rule: same-row calls are volume-only; an upward GainStep boundary calibrates on the first mixed body then sends the final body volume-only; a downward boundary sends the mixed body volume-only then calibrates on the final both-new body. VLLDP remains frozen and fail-quiet host ordering remains intact. The old v30 two-Q28 ordering evidence remains valid but its unconditional same-row calibration replay is superseded by v31. See `docs/findings/2026-08-18-GOLDEN-V31-CKV-DELTA-40HZ-PHYSICAL-GATE.md`.
- [x] **GREEN V05 — exact Windows final endpoint mute is live-proven and carried unchanged into v31.** Hash-matched qcadcm proves endpoint mute is a separate SetMute transaction at final VOL_CTRL iid `0x4a63`, pid `0x08001039`; selector `1/0` means mute/unmute. Linux builds the exact 104-byte Windows body behind the dedicated 12-byte TLV control, requires a running protected graph and serializes it under the endpoint lock. Successful exact-DSP mute/unmute no longer mirrors a redundant physical-sink mute; the hidden sink remains fail-closed fallback, and Golden v28/CPS retain downstream-only fallback because they lack this control. v31 changes only GainStep CKV-delta handling and retains the exact mute path.
- [x] **GREEN V06 — prior/new GainStep CKV delta semantics close the low-frequency Volume-Up control transient.** The v30 implementation resent the selected `0x489e` GainStep calibration on both left-new/right-old and both-new channel calls, including when the CKV did not change. Direct full-history q6apm-only 6%→46%→6% testing reproduced the defect without PipeWire (`UP HP500 p95 1.046e-2`, `DOWN 6.28e-4`), proving a stateful final-volume/GainStep boundary. v31 implements GSL-style changed-key deltas using a fixed final-volume-only control. On the unchanged fixed SP7 geometry, the real-key 40-Hz gate fell from v30 `2.7855e-3` UP p95 / `45.9x` UP:DOWN to v31 `6.6466e-5` / `0.975x`; an independent repeat measured `6.4095e-5` / `0.945x`. Native Windows measured `6.1937e-5` / `1.006x`. The correction changes no Dolby coefficient, WSA register, PA policy, SoundWire field, or ramp tuple. See `docs/findings/2026-08-18-GOLDEN-V31-CKV-DELTA-40HZ-PHYSICAL-GATE.md`.
...
- [~] **AMBER W02 — loopback-specific strict identity research item; no longer a built-in-speaker completion gate.** Raw WASAPI capture proves the Windows oracle is float32 and retains the same ~59.77 dB residual, but 432 consecutive raw float frames are byte-identical to a dedicated `audiodg` loopback branch and its `CCrossProcessServerOutputEndpoint` shared-memory ring. The loopback branch terminates at h42 in audiodg cross-process delivery, while the real speaker branch continues through Dolby/Surface/AudioLimiter to h59→h60 and an `AUDIOKSE.dll` device endpoint. At the loud 75-Hz region where the old residual is strongest, Windows↔Linux VR input/output, VLLDP input/output and a captured 480-frame AudioLimiter input/output block are bit-exact. Do not project the separate Windows loopback-client residual onto the physical speaker path. Full-file h60 literal identity is not claimed. See `docs/findings/2026-08-17-W02-WASAPI-LOOPBACK-BRANCH-NOT-SPEAKER-GATE.md`.
- [x] **GREEN W03 — v28 closes the downstream physical broadband static at the Windows/room floor.** The one-bit CPS-v3 A/B proved Windows `DRE_CTL_1.CSR_GAIN_EN=0` exposes the Linux floor only when the COMP sideband is malformed; a matched B1 matrix then identified WSA8845 DP2/COMP `OffsetCtrl2=0x07` as the missing prerequisite. Carrying only that field onto the full v27 Windows-lifecycle stack produces v28: first 1%-muted zero measured `2.467e-5` SP7-external-mic diff-RMS and a second wake after teardown + 20 s idle measured `1.818e-5`, versus retained Windows `1.825e-5`. v27 had been `0.002126/0.002350`, so v28 is roughly 86–129x quieter with no sustained PA floor. Codec init count stayed at two total across idle and no WSA/SoundWire/XRUN fault appeared. See `docs/findings/2026-08-17-DP2-OFFSETCTRL2-CSR-OFF-QUIET-CLOSURE.md`, `docs/findings/2026-08-17-V28-DP2-OFFSETCTRL2-FULL-STACK-STATIC-CLOSURE.md`, and patch `0065`. v30's DP1/DP3 structural completion preserves that closure: the matched SP7-external 1%-muted zero capture measured `1.8148e-5` whole-capture combined diff-RMS versus `1.8189e-5` from the retained v28 repeat under the same recalculation and `1.8253e-5` retained Windows reference.
...
- [x] **GREEN L03 — corrected seek path is structurally and physically closed on Golden v28.** Fresh SP7 KDNET now proves three steady-state Windows seeks cause zero runtime hits at qcad `SetVolume`, `GetGraphCkv`, or filtered SET_CFG for `0x4663/4664/4669/466b/4a63/489e/48a1/412b`; the same armed probes fire immediately on a 25%->17%->25% volume positive control. Thus Windows has no hidden host seek transaction to reproduce. Linux already carries the exact POPLESS calibration/headroom link and surrounding ramp policies, and a fresh corrected-topology 25% seek run produced no volume transaction, DSP/XRUN/SoundWire/WSA/PA fault while every obtained amp sample remained healthy. A synchronized local MediaPlayer Windows loopback shows a short near-silence transition, but prior real Edge/YouTube Windows loopback reaches ~0.987 full scale after seek, so that app-specific dip is not promoted into a guessed Linux fade. The old physical RED predates the pre-Dolby-volume, VLLDP-lifecycle and per-channel final-volume fixes and cannot be projected onto the current candidate. See `docs/findings/2026-08-15-WINDOWS-SEEK-KDNET-NO-HOST-CONTROL.md`. A fresh v28 25% three-seek run also produced zero XRUN/APM/WSA/SoundWire/PA faults and an SP7-external waveform with no seek transient beyond the surrounding program material's local p99.99 scale; see `docs/findings/2026-08-17-V28-SEEK-SP7-EXTERNAL-PHYSICAL-GATE.md`. The remaining subjective gate was subsequently passed: during normal manual YouTube use on the exact Golden v28 checkpoint the user reported that the prior forward/reverse loudness spike was no longer apparent. This closes L03 without inventing an additional seek actuator.
...
- [x] **GREEN L03b — exact Windows GainStep transaction, maximum-row transport, and prior/new CKV application are live-proven.** The full REV_0D sweep identifies the four ordered `0x489e` records `0x08001020`, `0x08001021`, `0x08001022`, `0x08001026`. The strict target/padding/four-frame validation and all 216/272-byte row classes remain unchanged. The important lifecycle correction is that this OOB group follows final VOL_CTRL **only when the prior/new CKV delta contains a changed GainStep key**; same-CKV channel calls are volume-only. v31 exposes a fixed 16-byte stereo volume-only actuator and live logs prove `vol->vol`, `cal->vol`, and `vol->cal` cases with no runtime transaction failure. This closes the semantic gap that produced the v30 40-Hz Volume-Up transient.
...
- [x] **GREEN S01 — clean integration source of truth identified.** Stable published integration is `/home/geoca/Documents/SP11-PROJECT/01-audio-cps-review`; Golden v31 is the saved/default deployment reference. Current post-promotion RX84 psychoacoustic-bass work is isolated on `agent/psycho-bass-20260818` pending the operator listening gate.
...
- [x] **GREEN S03 — remote publication and candidate preservation.** GitHub publication is healthy; `main` contains the promoted Golden-v31 reference while the active-RX84 userspace candidate, tests, physical findings and reviewed artifacts are pushed on `agent/psycho-bass-20260818`. All validation is local; no GitHub Actions runner is required. Generated/private vendor bytes remain excluded from Git.
...
1. **Active-RX84 operator promotion verdict:** Windows register provenance plus v31 physical 40-Hz, bass-transfer, exact-mute, seek and lifecycle gates are GREEN. Manually audition normal music/YouTube, bass balance, mute/unmute and volume before merging the userspace policy from `agent/psycho-bass-20260818` into Golden/main.
2. **Residual RAW L/R / upper-bass characterization:** use the fixed keyboard-length SP7 fixture with the hardened RAW recorder and `-ExpectedEndpointDb 0`. The large low-bass gap is already localized to the old RX81/-3 dB producer policy; do not add guessed EQ, Bass Enhancer or Virtual Bass for any smaller residual.
3. **Clean-source packaging:** normalize the historical Phase91 + Linux 7.1.5 platform base into a replayable public patch series; current hashes and incremental audio patches are evidence-complete but a pristine-upstream one-command rebuild is not yet claimed.
4. **W02 and private telemetry semantics (research):** continue only if useful; neither is a current physical-speaker quality blocker.
