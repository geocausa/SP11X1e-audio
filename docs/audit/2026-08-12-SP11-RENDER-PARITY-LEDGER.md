...
- [ ] **RED H03 — WSA DRE/CSR fallback lifecycle is not yet Windows-equivalent.** Fresh Windows KDNET proves both amplifiers initialize `DRE_CTL_1 0x34b1=0x00` and perform zero runtime writes to it across ordinary PA cycles, while the native Linux unmute path later forces `CSR_GAIN_EN=1`. The isolated Linux DRE-disabled cold-boot candidate produced ugly/unsafe speaker noise and is permanently rejected. Subsequent passive evidence closes several surrounding theories: the WSA-macro COMP producer finishes ~28.9 ms before PA unmute; 37 selected WSA TOP/RX/BOOST/COMP registers are identical before and after Linux's first-MCLK `regcache_sync()`; Windows native speaker start/stop sends no hidden qcadcm codec-register resource payload; and Denali/Hamoa maps DP2/COMP to the exact Windows left/right master ports, intervals, offsets and four-channel mask. The remaining gap is therefore the active WSA-macro COMP producer/runtime semantics or another downstream dependency, not startup ordering, first-sync overwrite, host resource injection or DP2 routing. Production remains on the known-good CPS-v3 CSR-assisted state. See `docs/findings/2026-08-15-WSA-HARDWARE-LIFECYCLE-REOPEN.md`, `docs/findings/2026-08-15-WSA-PRESYNC-STATE-CLOSEOUT.md`, and `docs/findings/2026-08-15-WINDOWS-HW-RESOURCE-AND-COMP-ROUTE-CLOSEOUT.md`.
