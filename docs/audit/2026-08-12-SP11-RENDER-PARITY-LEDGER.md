...
1. **H03 — explain the residual DRE/CSR coupling, not by value sweeping:** v5 CSR-enable-off is bounded-safe and directionally improves its matched predecessor. v6 separately zeroed the stored CSR gain field and is rejected because five exact-oracle runs became wildly non-repeatable despite clean PA/PCM lifecycle. Determine which remaining producer/DRE semantic makes `CSR_GAIN[5:1]` functionally relevant while `CSR_GAIN_EN=0`; do not re-arm v6 or optimize arbitrary gain codes.
