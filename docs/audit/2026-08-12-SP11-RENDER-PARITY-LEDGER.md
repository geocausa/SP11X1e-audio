...
- [~] **AMBER L03b — exact Windows GainStep transaction is implemented, live transport-tested on the prior candidate, and consolidated in Render-Parity; physical validation remains.** The full REV_0D sweep and recovered GSL/ACDB semantics identify the four ordered `0x489e` records `0x08001020`, `0x08001021`, `0x08001022`, `0x08001026`, sent as one 216/272-byte OOB APM SET_CFG after final `VOL_CTRL`. Patch `0048` plus capacity correction `0049` structurally validates and sends that complete group through the protected graph's existing OOB mapping; arbitrary targets, malformed payloads, nonzero padding, and idle graphs are rejected. The prior candidate accepted repeated 216-byte and all tested 272-byte rows live. The new consolidated candidate passes exact-release build/provenance, signing, initramfs and Wi-Fi bake checks. A Render-Parity boot plus physical slider/seek listening still gate GREEN. See `docs/deployment/2026-08-14-render-parity-candidate.md`.
- [~] **AMBER L04 — Windows SOFT_PAUSE lifecycle is evidence-exact and now installed only in the isolated Render-Parity candidate; runtime lifecycle validation remains.** Windows DEFAULT uses iid `0x466b`. Driver disassembly recovers tag `0x04010008`, key `0x01000021`, PAUSE/state-3 -> value `1`, RUN/release/state-4 -> value `0`; STOP also releases any outstanding pause. REV_0D for render subgraph `0xb000007e` maps value `1` to pid `0x0800102e` with zero-length payload and value `0` to pid `0x0800102f` with zero-length payload. PAUSE_COMPLETE `0x0800103f` and RESUME_COMPLETE `0x08001043` are registered separately, and the 50 ms completion bound derives from 20 ms rampdown + 25 ms downstream delay + 5 ms allowance. The `7.1.5-sp11-render-parity+` q6apm modules are signed and packaged; pause/resume/STOP/reprepare evidence after boot still gates GREEN.
...
- [~] **AMBER L07 — cold first-playback latency root cause is localized and its conservative correction is staged in Render-Parity.** Muted local playback reproduced 18.627/18.464 s launches and tracing showed four 4.28-4.60 s WSA-scale `regcache_sync()` calls: runtime-resume plus later attach-side replay for each amplifier. The candidate removes only unconditional `regcache_mark_dirty()` from ordinary clock-stop runtime suspend. Cache-only writes still become dirty, and UNATTACHED/ATTACHED retains the full context-loss restore. The resulting `snd-soc-wsa884x` has exact new vermagic and signature in the isolated candidate. Muted cold-start timing, attach recovery and suspend/resume after boot remain the gates; this change does not claim to solve warm seek/slider spikes. See `docs/findings/2026-08-13-WSA884X-COLD-START-DOUBLE-REGCACHE-REPLAY.md` and `docs/deployment/2026-08-14-render-parity-candidate.md`.
