...
- [ ] **AMBER H03 — WSA DRE/CSR consumer state now passes the isolated CSR-off bounded safety gate under the corrected Windows-like idle lifecycle; exact acoustic/state closure remains open.** Windows initializes both WSA8845s with `DRE_CTL_1=0x00` and performs zero runtime writes to it across ordinary PA cycles. The safe Linux baseline re-enables `CSR_GAIN_EN` on unmute. The old raw forced-zero candidate remains permanently rejected, and `csren0-v4` remains a genuine delayed-crackle failure under the old split-Dolby topology that incorrectly held PA/PCM RUNNING at idle. After commit `1523015` makes the hidden engine input passive and commit `2643e44` pins the actual Dolby bridge, the provenance-clean `csren0-v5-idlegated` one-bit candidate passed real-hardware 1% and 5% MP3 runs, three full 12% sweeps, correct producer-before-PA/PA-before-teardown ordering, and a 90-second idle gate with PCM `closed` for all 90 samples and no new WSA/PA/SoundWire/XRUN/ADSP faults. This is sufficient to remove H03 RED, but not to mark GREEN: a five-run byte-identical Windows-oracle comparison now shows CSR-off directionally improves the otherwise-matched no-HD2 v3 state in the stable 1--5 kHz band (MAE about 0.527 -> 0.432 dB; 7/8 points improve) while the broader 630 Hz--6.3 kHz aggregate remains essentially neutral. The stored CSR gain field has now been isolated separately: forcing its code from 7 to the Windows zero on top of v5 passes low-level 1%/5% and 90-second idle gates but catastrophically fails five-run 12% acoustic repeatability (v6 1--5 kHz pointwise spread ~10.2 dB median versus ~1.1 dB on v5). v6 is rejected; the remaining H03 question is why `CSR_GAIN` remains functionally coupled with `CSR_GAIN_EN` clear, implying a still-missing surrounding DRE/producer semantic rather than a simple register-value mismatch. See `docs/findings/2026-08-16-WSA-CSR-EN0-IDLEGATED-SAFETY.md`.
...
1. **H03 — explain the residual DRE/CSR coupling, not by value sweeping:** v5 CSR-enable-off is bounded-safe and directionally improves its matched predecessor. v6 separately zeroed the stored CSR gain field and is rejected because five exact-oracle runs became wildly non-repeatable despite clean PA/PCM lifecycle. Read-only regmap tracing now shows the missing distinction: UCM/profile construction writes `CSR_GAIN[5:1]` (code 7) before SoundWire resume, while ordinary demand playback never rewrites the gain field; only the CSR enable bit changes. v6 was therefore lifecycle-wrong because it forced code zero immediately at PA unmute. Qualcomm downstream WSA883x code independently sequences the gain field and enable bit as separate timed state. Next isolate Windows code zero at the existing profile/route write boundary on top of v5, with no unmute gain write; do not re-arm v6 or optimize arbitrary codes.
