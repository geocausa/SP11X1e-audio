...
**AMBER — Golden v28 remains the saved daily-driver/default reference and CPS-v3 remains the rescue entry. The isolated v31 CKV-delta candidate has passed the machine-verifiable v30 mute/DP1/DP3 inheritance gates, closes the pathological 40-Hz Volume-Up transient to the native-Windows/room-floor class on two independent SP7 captures, and retains the deterministic seek closure. Promotion is intentionally pending the operator's manual listening verdict. The principal remaining built-in-speaker physical-parity measurement is now a repeated **SP7 WASAPI-RAW** Windows-vs-Linux L/R and low-bass/psychoacoustic-bass A/B; the older shared-mode absolute L/R dB figures are provisional and must not drive tuning. P09/P10 and W02 remain explicitly non-blocking research/semantic items; clean public source packaging remains separate from sound-quality closure.**

## 1. Base Linux render path

- [x] **GREEN R01 — Kernel / ASoC platform stack.** Current live v31 candidate uses the accepted `7.1.5-sp11-render-parity-v4+` kernel/DTB with an isolated signed `snd-q6apm` initramfs override for endpoint-volume parity; X1E80100, q6apm/q6prm/APR, SoundWire and WSA884x are live. Saved/default boot remains Golden v28; CPS-v3 is retained as the rescue entry.
...
- [x] **GREEN S01 — clean integration source of truth identified.** Stable published integration is `/home/geoca/Documents/SP11-PROJECT/01-audio-cps-review`; Golden v28 is the saved/default deployment reference, and current post-Golden development is isolated on `agent/golden-v31-ckv-delta-20260818` until the operator listening gate is accepted.
...
- [x] **GREEN S03 — remote publication and candidate preservation.** GitHub authentication is repaired; `main` remains the consolidated Golden-v28 reference while the v31 candidate, patches, deployment verifier, tests, findings and reviewed artifacts are pushed on `agent/golden-v31-ckv-delta-20260818`. All v31 compile/sign/initrd/test work is local; no GitHub Actions runner is required. Generated/private vendor bytes remain excluded from Git.
...
1. **Golden-v30 user promotion verdict:** machine-verifiable exact-mute, DP1/DP3 transport, retention and physical-static gates are GREEN. Manually audition normal music/YouTube, mute/unmute, volume and seek behavior before changing the saved default from Golden v28.
2. **Bass / psychoacoustic-bass Windows A/B:** compare matched Windows and the accepted Golden candidate at the same apparent level. The recovered GainStep/MSIIR loudness contour is already present; do not add guessed bass processing unless the A/B proves a gap.
