...
**AMBER — the corrected built-in-speaker topology now has no still-valid, evidence-backed structural Windows/Linux mismatch in ordinary steady playback or live master-volume control. Historical physical failures were recorded before the unity-before-Dolby, VLLDP-lifecycle and channel-ordered endpoint fixes and therefore require fresh re-test. Do not call playback complete until W03/L03 receive a corrected-topology physical verdict and the seek-specific control path is rechecked.**
...
- [ ] **AMBER H03 — quiet CSR-off is now localized to coherent WSA8845 board/supply initialization history, not an isolated DRE or final current-limit value.** The fixed-geometry physical oracle remains decisive: Windows CSR-off active/non-zero `1.825e-5` median steady diff-RMS; Linux CPS-v3 CSR-on `1.862e-5` (`1.02x` Windows); exact v5 CSR-off `6.765e-4` (`37.1x`); v8 CSR-off `7.909e-4` (`43.3x`). Windows' one-second 997-Hz marker followed by continuous -80 dBFS 997 Hz proves its quiet result is an active path, not zero-sample power gating. The complete qcaucd oracle contains 63 ordered writes per amplifier and is left/right identical. A 2026-08-17 mechanical symbol audit against exact v5 `wsa884x.c` corrected four earlier labels while preserving every raw address/value/order: `0x34d2=CLSH_V_HD_PA`, `0x300b=REF_CTRL`, `0x3040=TOP_CTRL1`, and `0x306a=DAC_VCM_CTRL_REG7` (not `PA_FSM_TIMER0`; that is `0x3433`). v9 (remove early CSR enable), v10 (move DRE0=f0 into init), and v11 (explicit physical DRE1=0 init write) each remain v5-like noisy and are rejected, closing the simple DRE-history hypotheses individually. The first `ilim44-init-v12` was later proven **not to be an experiment**: its after-source and executable were v5, so its capture is only a v5 control. Provenance-clean v12R genuinely inserted `CURRENT_LIMIT=0x44` after `PWRSTAGE_CTRL2=0xf1`; its first PA cycle was comparatively quieter (~`1.98e-4` diff-RMS, still ~10.8x Windows), but a second identical zero-stream cycle on the same boot exploded to `3.8369e-3` (~5.67x v5 / ~210x Windows). v12R is therefore rejected for severe cycle/history instability. This is positive evidence that the manufacturer's analog sequence is stateful: Linux already reaches active `0x44` at PBR-enabled POST_PMU, so moving only the same final value earlier changed history rather than steady-state intent. **Do not continue isolated CURRENT_LIMIT/DRE value experiments.** v13 then replayed the full address-audited Windows positions 44--63 analog tail as one coherent pre-playback transaction. Cycle 1 improved materially to `3.175e-4` diff-RMS (~0.47x v5, still 17.4x Windows), but cycle 2 on the same boot/stimulus exploded to `3.927e-3` (~5.80x v5 / 215x Windows). v13 is rejected and cycle 3/program audio were cancelled. The leading gap is now **PA-cycle re-arm/teardown semantics**: the Windows cold tail can influence Linux first-wake state, but that state does not survive Linux teardown/reopen. A new Windows pass resolved the live `qcaucd8380.sys` base independently through `NtQuerySystemInformation` and armed the hash-locked `+0x1bf80` slave-helper breakpoint at its absolute code address, but three ordinary WASAPI cycles produced zero helper hits. Static qcaucd caller inventory then supplied the decisive scope result: the corrected analog-tail/current-limit/CKWD/VCM writes are confined to the codec-configuration function, while ordinary PA start/stop has separate Class-H/PA writers and does not replay that tail. v14 tested the strongest Linux-only stop mutation on top of v13: retain `PDM_WD_EN=1` across SP11 PRE_PMD as Windows does. That made cycle 1 itself catastrophic (`4.365e-3` diff-RMS, ~6.45x v5 / 239x Windows), so v14 is rejected with no further cycles. The gap therefore moves below isolated amp state to **SoundWire/producer transport preservation across PA-off idle**: Windows can retain initialized amp state under its sleep/clock-stop lifecycle, whereas Linux currently fully disables/deprepares the streams before PCM closes. Live qcaucd WPP now closes the next observation: Surface `SwrSleep=1` / `SwrClockStopTimerMS=500` is active, with delayed clock-stop bursts at +0.485..0.499 s on two repeated Windows cycles; dataport programming is replayed on both start and stop. Linux additionally frees the WSA slave/port runtimes and then the entire `sdw_stream_runtime` on each close. H03 therefore tested **topology-object retention with streams still disabled/deprepared and PA off**, not active dataport retention. v15 retained the stream/master/slave/port runtime nodes, reused all three WSA playback/VI/CPS runtimes, and still reached SoundWire runtime suspend with PCM closed; nevertheless the next mic-measured PA wake produced `2.867e-3` median diff-RMS (~4.24x v5 / 157x Windows). v15 is rejected. The gap is now below C object lifetime, in the physical SoundWire clock-stop/slave/master or adjacent producer low-power state transition. v16 then matched qcaucd's next proven master-side detail: after the same Mode-0 broadcast `SCP_CTRL=CLK_STOP_NOW`, wait on `SWRM_COMP_STATUS[0]` at ~1-ms cadence before gating `hclk`. The wait is physically meaningful on SP11 and completed cleanly, but the subsequent zero-stream wake still measured `2.106e-3` median diff-RMS (~115x Windows / 3.11x v5). This is materially better than v13 cycle 2 (`0.536x`) and v15 (`0.735x`), so post-stop completion ordering is a real contributor but not sufficient. v17 then skipped Linux's LPASS AudioCC WSA CGCR reset pulse on normal resume; link/runtime-PM remained healthy, but the next wake regressed to `2.841e-3` (~1.35x v16), so the CGCR pulse is retained and is not the missing Windows mismatch. See `docs/findings/2026-08-16-CSR-OFF-ACTIVE-NOISE-ORACLE.md`, `docs/findings/2026-08-16-WINDOWS-WSA8845-FULL-INIT-HISTORY.md`, `docs/findings/2026-08-17-WSA-CURRENT-LIMIT-INIT-V12R-REJECTION.md`, and `artifacts/reviewed/2026-08-17-v12r-current-limit-init-instability.json`.
...
- [~] **AMBER W03 — corrected current topology awaits a fresh physical Windows-parity verdict.** The earlier RED listening failure was recorded on a topology that was later proved to attenuate PCM before Dolby and to live-retune VLLDP on every slider change, both unlike Windows. Those defects are now corrected: unity PCM feeds the recovered Movie Dolby engine, the fresh production path matches its Windows-backed oracle at ~0.99995/0.99990 correlation, VLLDP is generation-frozen, and live final VOL_CTRL/CKV follows fresh Windows KDNET. The old audible verdict must not be projected onto this new topology; user listening remains the final gate before GREEN.
...
1. **H03 — test Windows 500-ms SoundWire idle timing on top of v16:** v17 isolated Linux's normal-resume LPASS AudioCC WSA CGCR reset pulse. Skipping it is structurally viable but worsens repeat-cycle noise to `2.841e-3` (~156x Windows, 1.35x v16), so the normal CGCR pulse stays. The remaining independently live-proven master-lifecycle delta is Surface `SwrClockStopTimerMS=500` versus qcom Linux 3000-ms autosuspend. Test 500 ms while retaining v16's post-stop frame-generator completion wait and otherwise exact v13 behavior. See `docs/findings/2026-08-17-SOUNDWIRE-CGCR-RESET-V17-REJECTION.md`.
2. **W03 — obtain a fresh physical verdict on the corrected topology:** the previous “not close to Windows” judgment predates the unity-before-Dolby fix, VLLDP lifecycle correction, exact channel-ordered endpoint actuator and now the Windows-like PA-at-idle shutdown. If steady tonality still fails, continue matched downstream-state investigation without inventing an unobserved processing block.
3. **L03 — isolate the seek/discontinuity-specific physical smoothing:** ordinary live volume sequencing (L03a/V04/L03b), SOFT_PAUSE and the pause-drain path are structurally closed. Re-capture a matched seek on the corrected topology and compare Windows qcad/AudioReach control activity around the discontinuity rather than reopening the ordinary slider path.
4. **W02 — strict digital identity:** revisit only after the current physical gates are resolved; the existing ~60 dB residual is too small to justify a large guessed coloration stage.
5. Publish/merge the clean integration branch and archive obsolete contradictory docs.
