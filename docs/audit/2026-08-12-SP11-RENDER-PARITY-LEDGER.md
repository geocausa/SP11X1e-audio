...
1. **W03/H05/H08/P06/H07 — live-gate the installed DP5/VISENSE candidate:** the retained 328-write qcaucd FIFO log is fully decoded. Windows uses DP5 mask `0x03` on each amplifier; running Linux uses `0x01`. The Denali-scoped normal-SoundWire override is built, signed, installed as isolated one-shot `sp11-audio-visense-parity`, and offline platform/DTB/initramfs/GRUB gates pass. Require clean master-port-10/11 allocation plus SP/SPVI startup, then perform a bounded listening comparison. DP4 is closed negative for ordinary playback and remains unscheduled. No new Windows capture is needed.
