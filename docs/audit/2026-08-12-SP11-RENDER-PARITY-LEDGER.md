...
- [x] **GREEN H03 — exact shipping Windows WSA8845 three-state lifecycle is implemented and live-proven on Linux.** Candidate v26 makes the Denali profile cold-init once with the recovered 63-write oracle, suppresses Linux-only ordinary DAPM gain/DRE/current/PDM-watchdog mutations, and executes only the exact 10-write START plus 6-write STOP at `mute_stream()`. A dedicated register-write trace over three complete natural idle/play/idle cycles captured exactly 96 WSA writes = 3 × 2 amps × 16; every START/STOP sequence matched the Windows oracle, with zero extra WSA mutations, zero relevant kernel fault/XRUN lines, both SoundWire slaves suspended and ALSA PCM closed afterward. See `docs/findings/2026-08-17-LINUX-WSA8845-WINDOWS-THREE-STATE-V26.md`, `artifacts/reviewed/2026-08-17-linux-wsa8845-three-state-v26-validation.json`, and patch `0063`.
...
1. **W03 — obtain a fresh physical verdict on the corrected v26 topology:** the previous “not close to Windows” judgment predates the unity-before-Dolby fix, VLLDP lifecycle correction, exact channel-ordered endpoint actuator, Windows-like PA-at-idle shutdown and now the exact live-proven WSA8845 three-state lifecycle.
