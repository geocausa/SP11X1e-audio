...
- [ ] **RED H03 — WSA DRE/CSR fallback lifecycle remains the final known consumer-side hardware gap, but the host lifecycle that invalidated the first CSR-off test is now fixed.** Windows initializes both WSA8845s with `DRE_CTL_1=0x00` and performs zero runtime writes to it across ordinary PA cycles; Linux safe baseline still re-enables `CSR_GAIN_EN` on unmute. Windows producer state is now directly recovered far beyond the earlier H03 wording: RX84/0 dB, primary half-dB off, Surface compander curve, VBAT/BCL, TOP_CFG1/base bits and no generic HD2 are all evidence-backed. A one-variable `csren0-v4` candidate changed only the 2S unmute CSR-enable bit and carried controlled short 1% program audio, but later produced prolonged crackling and is rejected. Crucially, that boot used the August-14 split Dolby graph which was proven to keep the physical ALSA PCM and PA RUNNING indefinitely with no application streams, unlike Windows. The hidden Dolby engine input is now passive; safe CPS-v3 proves demand wake, PA-on only during playback, PA mute before producer teardown, SoundWire deprepare and PCM `closed` at idle. Do **not** re-arm the old candidate unchanged as a production oracle; the next H03 gate is a fresh bounded CSR-off safety test under this corrected host lifecycle, followed by matched acoustics only if idle/reopen remains clean. See `docs/findings/2026-08-16-WSA-CSR-EN0-DELAYED-CRACKLE.md` and `docs/findings/2026-08-16-DOLBY-IDLE-PA-LIFECYCLE-PARITY.md`.
...
1. **H03 — re-open CSR-off only under the corrected idle lifecycle:** the old forced-zero candidate and the later one-variable `csren0-v4` delayed-crackle boot remain rejected. The next experiment must use the now-passive host graph, a low endpoint setting, bounded deterministic playback, an idle dwell long enough to cover the former failure window, and automatic fail-back to CSR-assisted CPS-v3 if any crackle/PA fault appears.
