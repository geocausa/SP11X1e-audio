...
- [ ] **AMBER H03 — WSA DRE/CSR consumer semantics remain open; both exact-zero variants are rejected.** Windows initializes both WSA8845s with `DRE_CTL_1=0x00` and performs zero ordinary runtime writes to that register. v5 proved that keeping `CSR_GAIN_EN` clear under the corrected passive-idle host lifecycle is bounded-safe and directionally improves the matched no-HD2 predecessor (five-run 1--5 kHz MAE about 0.527 -> 0.432 dB) while leaving the stored gain code at 7. v6 is permanently rejected because it forced `CSR_GAIN[5:1]=0` at PA unmute and produced wildly non-repeatable 12% response. Read-only regmap tracing then proved Linux normally writes the gain field during UCM/profile construction before SoundWire resume, so v7 moved zero to that natural route boundary with a boot-scoped PA Volume 24 -> 31 overlay and no unmute gain write. v7 passed cold provenance, exact write-history, 1% real audio, a 90/90 closed-idle gate and the 5% lifecycle gate, but the first byte-identical 12% chirp produced operator-observed static/noise while active. A second SP7 recorder running with no playback stayed at the normal room/mic floor while PCM was closed, and v7 active-playback midband spectral flatness was about 3.2--3.5x the five-run v5 median. v7 is therefore rejected and the remaining four 12% runs were cancelled. Do not re-arm v6 or v7 and do not promote PA Volume 31. The unresolved question is why Windows can retain clean exact-zero DRE/CSR state without the Linux acoustic failure; pursue surrounding DRE/Class-H/producer semantics rather than value sweeping. See `docs/findings/2026-08-16-WSA-CSR-ROUTE-ZERO-V7-CHECKPOINT.md` and `artifacts/reviewed/2026-08-16-v7-active-static-rejection.json`.
...
1. **H03 — finish the route-time-zero v7 gate, without value sweeping:** v7 now matches the Windows zero gain code at Linux's natural UCM/profile write boundary and avoids v6's lifecycle-wrong unmute rewrite. Cold boot, real 1% playback and the 90-second all-closed idle gate pass. Next run the bounded 5% MP3 gate and then the byte-identical 12% five-run Windows oracle. Any crackle/non-repeatability rejects v7; do not re-arm v6 or invent intermediate gain codes.
