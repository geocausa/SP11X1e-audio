...
- [~] **AMBER L03 — historical real-YouTube seek spike requires fresh corrected-topology validation.** The Aug-13 Linux/Windows captures remain useful localization evidence: both OS loopback/host boundaries can approach full scale on re-entry while Windows physical output was heard as smoothly capped, and the fourth `POPLESS_EQ 0x4664 <-> VOL_CTRL 0x4663` headroom link is structurally required. However, the physical Linux failure was auditioned before three later-proved defects were corrected: PCM was attenuated before Dolby, VLLDP was incorrectly live-retuned by endpoint volume, and final master volume was emitted as one simultaneous stereo update instead of Windows' two qcad SetVolume calls. The old seek verdict therefore cannot be treated as a current RED. Ordinary master-volume sequencing is now GREEN in L03a/V04/L03b. Next work is a fresh matched seek on this corrected topology plus Windows KD/qcad control capture around the discontinuity; do not reopen ordinary slider logic or add a guessed smoothing effect.
