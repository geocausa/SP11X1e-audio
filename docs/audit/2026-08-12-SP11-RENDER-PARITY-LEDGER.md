...
- [ ] **AMBER H02 — LPASS WSA-macro digital gain is operationally isolated but not yet Windows-proven.** The exact CPS-v3 runtime trace shows UCM/WirePlumber writes `RX0_RX_VOL_CTL=RX1_RX_VOL_CTL=0xfd`, ALSA value 81 / **-3 dB**. The generic X1E machine driver documents that 81 limit as a temporary speaker-damage cap until active protection exists. A provenance-clean one-shot candidate patched only the four accepted machine-driver `snd_soc_limit_volume(...,81)` instruction immediates to 84, keeping kernel/DTB/WSA-macro/WSA8845/q6apm/SoundWire/DRE unchanged. With CSR-assisted mode retained, explicit 84/0 dB tests at endpoint 5% and 12% completed with zero WSA/PA/SoundWire/XRUN faults; both amps were observed PA-enabled with current-limit code 17 and zero PA errors, then the macro gain was restored to 81. This proves 0 dB is safe on the current protected Linux stack, **not** that Windows uses 0 dB. The common first-MCLK pre-state is `0x00`, Windows uses the same qcadsp, and fresh qcadcm tracing shows no host WSA gain table, but Windows firmware-internal runtime writes remain possible. See `docs/findings/2026-08-15-WSA-MACRO-DIGITAL-GAIN-ISOLATION.md`.
- [ ] **RED H03 — WSA DRE/CSR fallback lifecycle is not yet Windows-equivalent.** Fresh Windows KDNET proves both amplifiers initialize `DRE_CTL_1 0x34b1=0x00` and perform zero runtime writes to it across ordinary PA cycles, while native Linux unmute later forces `CSR_GAIN_EN=1`. The isolated Linux DRE-disabled cold-boot candidate produced ugly/unsafe speaker noise and is permanently rejected. A fresh boot-bounded `_regmap_write()` trace now positively proves the native safe CSR lifecycle on untouched CPS-v3: `alsactl` restores `0x34b1=0x0f`, COMP-aware speaker POST_PMU writes `0x0e`, and the real PipeWire data-loop unmute writes `0x0f` again immediately before `PA_FSM_EN=1` on each amplifier. This closes the previous source-only inference that generic Linux unmute is the actuator re-enabling CSR assistance; Windows has no corresponding runtime `0x34b1` rewrite. The exact loaded CPS-v3 producer transition is also known and producer timing is closed: final macro POST_PMU completes **29.769 ms before first WSA8845 unmute**. The generic WSA-macro half-dB policy has also been isolated on the exact `F681...` binary: a one-instruction candidate removed only the four POST_PMU `mask=1,value=1` updates at `0x428/0x444/0x4a8/0x4c4`; live baseline-vs-candidate trace showed 24 removed writes and **zero added operations**, with COMP/Boost/RX-path operations unchanged. Bounded 5% and 12% playback remained PA/current-limit clean with zero speaker faults. Thus the half-dB policy is not required for safe CSR-assisted operation, but its Windows state remains unknown. Together with H02's separately isolated -3 dB macro cap, the remaining RED gap is specifically **which active producer gain/COMP semantics Windows uses and why Linux COMP-only mode fails when CSR assistance is removed**. Production remains on accepted CPS-v3 CSR-assisted mode; do not combine producer changes with DRE=0 without a new evidence gate. See `docs/findings/2026-08-15-WSA-HALFDB-POLICY-ISOLATION.md` and `docs/findings/2026-08-15-WSA-CSR-UNMUTE-RUNTIME-PROOF.md`.
...
1. **W03 — obtain a fresh physical verdict on the corrected topology:** the previous “not close to Windows” judgment predates the unity-before-Dolby fix, VLLDP lifecycle correction and exact channel-ordered endpoint actuator. If steady tonality still fails now, continue matched downstream-state investigation without inventing an unobserved processing block.
2. **L03 — isolate the seek/discontinuity-specific physical smoothing:** ordinary live volume sequencing (L03a/V04/L03b), SOFT_PAUSE and the pause-drain path are now structurally closed. Re-capture a matched seek on the corrected topology and compare Windows qcad/AudioReach control activity around the discontinuity rather than reopening the ordinary slider path.
