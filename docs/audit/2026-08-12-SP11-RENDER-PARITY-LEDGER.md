...
1. **H03 — finish CSR-off acoustic/exact-state closure:** `csren0-v5-idlegated` has passed the corrected-lifecycle bounded safety gate through 12% plus a 90-second all-closed idle dwell. Repeat the synchronized acoustic oracle with the recovered byte-identical Windows chirp; only if that result is directionally sound should the stored CSR gain field be isolated as a separate final state variable. The raw forced-zero and old v4 delayed-crackle candidates remain rejected.
