...
- [ ] **RED V04 — Windows final endpoint-gain actuator/transition boundary is not yet reproduced; its DSP ramp policy is now exactly identified.** The preserved Windows KD capture sends live `VOL_CTRL 0x4a63 / PARAM_ID_VOL_CTRL_MULTICHANNEL_GAIN 0x08001038` bodies whose Q28 gains match the independently recovered Windows endpoint taper: `0x0013615a = -46.50 dB` (~4% UI) and `0x007dda19 = -30.25 dB` (~13.04% UI). In one DEFAULT interval Windows emits two extra `0x4a63` gain writes without another DEFAULT graph open: first L=new/R=old, then both=new, followed by a NOTIFICATION graph open. This is strong evidence of live per-channel endpoint-volume/control updates, although the original July operator action is not timestamped well enough to call it a proven slider gesture. Exact qcadcm `SetVolume` decompilation now identifies adjacent param `0x08001037` as the gain-ramping policy with fields `period_ms`, `step_us`, and `ramping_curve`; the exact booted Linux topology already loads final `0x4a63` with Windows payload `10, 1000, 3`. Thus the smoothing policy itself is present, but Linux never exercises it for user volume because final `0x4a63` is left at Q28 unity and endpoint attenuation is applied in the hidden PipeWire/ALSA sink. The protected topology exposes no `0x4a63` volume control despite generic runtime DSP-volume transport existing in the clean kernel lineage. Therefore V01/V02 stay GREEN for gain law/shared state, while actuator/transition-boundary parity is RED. An isolated fixed-target `0x4a63/0x08001038` candidate reproduces the captured 104-byte Windows bodies exactly, passes strict checkpatch, builds with exact `7.1.5-sp11-cps-v3+` vermagic, and remains unsigned/uninstalled. See `docs/findings/2026-08-13-WINDOWS-FINAL-VOLCTRL-RUNTIME-ACTUATOR-GAP.md` and `artifacts/reviewed/2026-08-13-final-volctrl-ramping-policy.json`.
...
2. **V04/L03/L03a — close the general physical transient:** the four-link POPLESS/headroom restoration failed the physical seek gate. Finish the pending fixed-volume MSIIR listening discriminator and, in a live window, A/B the current host-side endpoint attenuation against the exact Windows final-`VOL_CTRL 0x4a63` actuator while keeping the taper/Dolby/MSIIR state identical. Do not re-run already-closed digital localization or revive superseded limiter assumptions.
