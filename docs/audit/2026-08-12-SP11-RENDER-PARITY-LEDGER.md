...
2. **W02 — Windows loopback branch identity (non-blocking):** the ~60 dB residual belongs to a dedicated audiodg/WASAPI loopback branch, not the physical speaker completion path. Continue only as a separate engine-research item; do not fit speaker coloration to it.
