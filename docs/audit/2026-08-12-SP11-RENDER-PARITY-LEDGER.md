...
1. **H03 — isolate the quiet-CSR-off initialization dependency, not another value:** the complete 63-write Windows codec-init oracle and the four-way active-noise control are now available. Windows CSR-off and CPS-v3 CSR-on are equally quiet; Linux v5/v8 CSR-off are 37--43x noisier in diff-RMS. Compare the ordered Windows 2S/4-ohm DRE/watchdog/current-limit transitions against Linux boot/UCM/DAPM write history and rank the smallest stateful mismatch. Do not copy the whole Windows transaction blindly; any behavioral candidate must isolate one initialization transition and first pass a muted digital-silence/noise gate.
