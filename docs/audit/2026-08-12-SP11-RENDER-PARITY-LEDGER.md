...
**AMBER — Golden v28 remains the saved daily-driver reference and closes the confirmed broadband static plus seek/discontinuity gate. The isolated Golden-v30 candidate has now passed every machine-verifiable gate for exact Windows endpoint mute and DP1/DP3 slave-transport completion while preserving the v28 physical room-floor noise class and exact resident WSA lifecycle. v30 promotion is intentionally pending the user's manual listening verdict. After that, the principal physical-parity question is the direct Windows-vs-Linux low-bass / psychoacoustic-bass A/B; clean-source packaging also remains open. W02 stays a separate non-blocking WASAPI-loopback research item.**
...
- [x] **GREEN V05 — exact Windows final endpoint mute is live-proven on the isolated v30 candidate, with the downstream fail-safe retained.** Hash-matched qcadcm proves endpoint mute is a separate SetMute transaction at final VOL_CTRL iid `0x4a63`, pid `0x08001039`; selector `1/0` means mute/unmute. Linux v30 builds the exact 104-byte Windows body behind a dedicated 12-byte TLV control, requires a running protected graph and serializes it under the existing endpoint lock. Direct `1→0→1` writes all returned rc=0. Once the running baseline was established, live desktop mute-only/unmute-only gestures hit `audioreach_sp11_set_final_mute()` without any final-volume call, while the hidden sink remains a fail-closed backstop. Golden v28/CPS automatically retain the downstream-only fallback because they do not expose the new control. Focused tests: 33/33; broader volume/Dolby regression: 54 tests + 6 subtests. Promotion to the saved daily driver remains gated on the user's v30 listening verdict, not on mute implementation evidence.
...
- [x] **GREEN S01 — clean integration source of truth identified.** Stable published integration is `/home/geoca/Documents/SP11-PROJECT/01-audio-cps-review` at main/render-parity; Golden-v30 development is isolated on `agent/golden-v30-mute-transport-20260818` until its user listening gate is accepted.
