...
- [ ] **AMBER H03 — the complete shipping Windows WSA8845 codec state machine is now live-captured and decompiled; Linux ordinary DAPM lifecycle is structurally wrong.** Software-only KDNET attached before qcaucd bring-up, qcaucd WPP, and Ghidra of the exact live `qcaucd8380.sys` (v1.0.0.10344, SHA `BD0C8276...`) prove one three-state Windows codec policy: state 0 = a 63-write cold init once per amp; state 1 = an exact 10-write ordinary START; state 2 = an exact 6-write ordinary STOP. Across boot/login plus two real MP3 start/stop cycles, each amp has exactly 63 init writes, 3x10 START writes, 3x6 STOP writes and **zero other WSA8845 writes**; left/right are identical. Windows ordinary runtime never rewrites DSM/DRE/watchdog/current-limit/CKWD/analog-init state. Exact v13 Linux instead re-runs gain/DRE/current/PDM policy in POST_PMU/PRE_PMD, toggles DRE CSR in `mute_stream()`, and implements only part of the Windows PA transaction. v21 therefore remains a useful diagnostic proof that state loss matters, but its per-cycle 20-write analog-tail replay is not the Windows lifecycle and must not be promoted as final policy. The next implementation is an SP11-specific WSA8845 profile: cold init once, exact 10-write START and 6-write STOP at `mute_stream()`, with generic non-SP11 behavior unchanged. See `docs/findings/2026-08-17-WINDOWS-WSA8845-COMPLETE-LIFECYCLE-STATE-MACHINE.md` and `artifacts/reviewed/2026-08-17-windows-wsa8845-complete-state-machine-oracle.json`.
...
1. **H03 — implement the captured Windows WSA8845 three-state profile, not another value experiment:** state 0 is the one-time 63-write SP11 codec/board init; state 1 is the exact 10-write START; state 2 is the exact 6-write STOP. First remove Linux-only normal-cycle DRE/current/PDM-watchdog/CSR mutations from the SP11 runtime path and make `mute_stream()` reproduce START/STOP exactly. Gate structurally with a read-only codec-write trace: per amp one START must be exactly 10 writes and one STOP exactly 6, with zero cold-only writes. Only after that gate passes return to the 1% muted digital-zero acoustic oracle.
