...
- [ ] **AMBER H03 — WSA DRE/CSR consumer semantics remain open; v7 is a valid exact-zero rejection and the old v6 causal attribution is retracted.** Windows initializes both WSA8845s with `DRE_CTL_1=0x00` and performs zero ordinary runtime writes to that register. v5 proved that keeping `CSR_GAIN_EN` clear under the corrected passive-idle host lifecycle is bounded-safe and directionally improves the matched no-HD2 predecessor (five-run 1--5 kHz MAE about 0.527 -> 0.432 dB) while leaving the stored gain code at 7. A later Kbuild provenance audit found that the packaged v6 module was copied from a stale prepared-output path: its `wsa884x_mute_stream()` machine code is identical to v5 and therefore does **not** contain the intended extra `CSR_GAIN[5:1]=0` unmute write. The v6 captures remain historical measurements but cannot support the previous gain-zero/unmute causal conclusion. v7 remains valid because it deliberately reused the proven v5 module and changed only the boot-scoped UCM PA Volume 24 -> 31 route-time programming; v7 passed cold/idle/1%/5% lifecycle gates but produced operator-observed active-playback static/noise on the first exact 12% chirp, with stable-midband spectral flatness about 3.2--3.5x the five-run v5 median, and is rejected. The next provenance-clean v8 candidate keeps v5's stored code 7 and exact Windows Class-H/power-stage transaction but removes only the redundant SP11 2S `DRE_CTL_1` writes inside ordinary `mute_stream()` start/stop, matching Windows' zero-runtime-write history without another value sweep. See `docs/findings/2026-08-16-WSA-V6-PROVENANCE-RETRACTION-AND-DRE-COLD-V8.md`, `docs/findings/2026-08-16-WSA-CSR-ROUTE-ZERO-V7-CHECKPOINT.md`, and `artifacts/reviewed/2026-08-16-v7-active-static-rejection.json`.
...
1. **H03 — validate provenance-clean DRE-cold v8 without value sweeping:** v7 is rejected for active-playback static/noise. The old v6 source-delta attribution is retracted because its packaged `mute_stream()` did not contain the intended gain-zero call. v8 keeps the proven v5 value/UCM/producer state and changes only ordinary SP11 2S DRE write history: no `DRE_CTL_1` write inside PA unmute/mute, while the exact Windows Class-H/power-stage/PA transaction remains. Start at 1% with read-only write tracing and reject immediately on static/crackle or lifecycle fault; only then proceed to idle dwell, 5%, and exact 12% oracle.
