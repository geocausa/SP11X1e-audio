...
1. **H03 — map the physical SoundWire clock-stop/slave state boundary:** v15 now closes topology-object lifetime as insufficient. It retained/reused the SP11 WSA playback/VI/CPS stream/master/slave/port runtimes while keeping PA/producer off, disable/deprepare intact, and the SoundWire devices runtime-suspended; the subsequent zero-stream wake was still catastrophically noisy (`2.867e-3`, ~157x Windows). Next compare qcaucd/QCSWR `SwrSleep` clock-stop/slave-state sequencing with Linux qcom SoundWire runtime suspend/exit-clock-stop and producer low-power state. No further amp-register or stream-object candidate until that physical transition is explicit. See `docs/findings/2026-08-17-WSA-SDW-TOPOLOGY-RETENTION-V15-REJECTION.md`.
