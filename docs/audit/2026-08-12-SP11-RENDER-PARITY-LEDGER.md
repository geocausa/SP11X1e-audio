...
- [~] **AMBER W02 — strict sample identity; fresh Windows now proves the speaker DSP chain bit-exact through AudioLimiter and moves the residual to the WASAPI loopback/tap boundary.** Fresh Windows repeated the Aug-12 clean oracle sample-for-sample. A full-memory dump in the loud 75-Hz region plus an instrumented exact Linux replay proves VR input/output, VLLDP input/output, Surface copy, and a captured 480-frame AudioLimiter input/output block bit-exact Windows↔Linux. The only real `CAudioRateConvertCMPT` node is an upstream equal-rate byte-copy. Terminal h59→h60 is proven float32→int16 via `ConvertFloat32ToInt16Dither_NEON`, but replaying its exact dither arithmetic for all 4096 cursors does not match the WASAPI loopback; therefore the loopback oracle is a different engine tap, not the device-format stream. Next gate: raw WASAPI packet capture + exact mix-format logging and direct tap-buffer localization. See `docs/findings/2026-08-17-W02-FRESH-WINDOWS-DUMP-BITEXACT-DSP-BOUNDARY.md`.
