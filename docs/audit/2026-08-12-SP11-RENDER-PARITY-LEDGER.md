...
1. **H03 — resolve SoundWire/producer state preservation across PA-off idle:** static qcaucd proves the Windows analog tail is configuration-time only; v13 loses its improved first-cycle state only after Linux teardown, and v14 proves keeping only the Windows-latched PDM watchdog while Linux still tears down transport is catastrophically wrong. Audit QCSWR0/qcaudminiport/qcaucd sleep/clock-stop policy (`SwrSleep=1`, `SwrClockStopTimerMS=500`) against Linux `hw_free -> sdw_disable_stream -> sdw_deprepare_stream -> runtime suspend`. Determine whether Windows retains dataport/stream configuration in clock-stop while PA is off. The next candidate must preserve PA-off safety while matching that lower transport boundary; no more isolated amp-register writes.
