...
1. **H03 — resolve the repeated-PA-cycle state loss:** v13 proves the coherent Windows cold analog tail materially improves Linux cycle 1 but fails catastrophically on cycle 2. Before another Linux candidate, capture repeated Windows PA cycles through the safe qcaucd slave-write helper with the filter expanded to the audited analog-tail registers (`3003/04/05/06/0b`, `3040/45-49/4c`, `306a`, `3090/91`, `30a0/a5/aa/ba`) plus the known PA/Class-H controls. Determine exactly what Windows re-arms or tears down per cycle; then reproduce that coherent boundary on SP11 2S. No direct MMIO and no program-audio escalation until repeated digital-zero cycles are both quiet and stable.
