...
1. **H03 — test the remaining explicit-init semantics, not final DRE values:** v9 closes transient CSR enable and v10 closes early DRE0 timing. Windows still differs by physically writing `DRE_CTL_1=0x00` inside the coherent init block; retired 0055 changed a software regmap default rather than issuing that transaction. Statistically rank that explicit init write against the later `CKWD 0x1b->0x13` / `CURRENT_LIMIT 0xd4->0x44` transitions. Any behavioral candidate must remain one-variable and first pass the muted digital-silence oracle.
