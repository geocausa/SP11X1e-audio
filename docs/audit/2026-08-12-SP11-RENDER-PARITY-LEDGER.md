...
**AMBER — Golden v28 closes the confirmed broadband static and the seek/discontinuity gate is now GREEN after both the SP7-external objective test and the user's direct YouTube listening verdict. The user reports stable/coherent music, excellent volume leveling and no audible forward/reverse spike on the preserved v28 state. Remaining speaker-parity work is now primarily the direct Windows-vs-Linux low-bass / psychoacoustic-bass A/B plus clean-source packaging. W02 remains a separate non-blocking WASAPI-loopback branch identity item.**
...
- [x] **GREEN V05 — desktop mute behavior is restored fail-safe without perturbing the Golden audio path.** The visible sink mute bit was already detected, but the running volume-transaction synchronizer did not actuate it downstream. The userspace synchronizer now establishes the hidden sink gain first and mirrors mute/unmute to that hidden endpoint. Focused tests: 29/29; broader volume/Dolby regression: 48 tests + 6 subtests; live control-plane test proved both visible and hidden sinks enter/leave `[MUTED]` together. Windows' exact runtime final-VOL_CTRL mute parameter `0x4a63/0x08001039` remains a separate non-blocking exact-parity enhancement; no DSP/WSA/PA state was changed for this operational fix.
...
1. **Golden v28 consolidation/reproducibility:** keep Golden v28 as the daily-driver default, CPS-v3 as rescue and v29 only as a structural comparison. Normalize the historical Phase91+7.1.5 platform base into a replayable clean patch series.
2. **Bass / psychoacoustic-bass Windows A/B:** compare matched Windows and Golden v28 at the same apparent level. The recovered GainStep/MSIIR loudness contour is already present; do not add guessed bass processing unless the A/B proves a gap.
3. **Exact runtime DSP mute (non-blocking):** user-facing mute is fixed downstream; promote the captured `0x4a63/0x08001039` transaction only as an isolated parity change.
