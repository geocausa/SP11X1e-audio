...
- [~] **AMBER W02 — strict sample identity; residual now localized to a microscopic LF complex-transfer/state difference.** Exact replay reproduces `0.99999947385 / 59.778 dB`; the error is concentrated around ~55/75-Hz program regions (~0.01 dB / ~0.1° scale below 100 Hz) and collapses above ~500 Hz. PCM16 loopback conversion, current AudioEng `CAudioCleanup`, the proven-transparent Microsoft VR→VLLDP interstitial path, and tested one-LSB Movie/VLLDP profile perturbations are rejected as explanations. Next exact gate is a fresh state-pinned Windows Movie `audiodg` dump plus loopback to localize the difference at VR/VLLDP boundaries. See `docs/findings/2026-08-17-W02-LF-RESIDUAL-AND-AUDIOCLEANUP-CLOSURE.md`.
