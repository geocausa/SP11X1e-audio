...
1. **H03 — continue into the coherent Windows DRE/watchdog/current-limit init block:** v9 closes the early transient CSR-enable hypothesis; its zero-stream noise is essentially identical to v5. Prioritize the smallest initialization-history mismatch that Windows establishes before `DRE_CTL_1=0`: `DRE_IDLE=0x0f -> PDM_WD=1 -> DRE_CTL_0=0xf0`, then the later `CKWD 0x1b->0x13` / `CURRENT_LIMIT 0xd4->0x44` transitions. Use static Qualcomm semantics and read-only write-history evidence to rank them; any behavioral candidate must isolate one transition and first pass the muted digital-silence oracle.
