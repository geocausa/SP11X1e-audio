...
- [ ] **AMBER H02 — LPASS WSA-macro digital gain is stable but not yet Windows-proven.** The exact CPS-v3 runtime trace shows UCM/WirePlumber writes `RX0_RX_VOL_CTL=RX1_RX_VOL_CTL=0xfd`, corresponding to ALSA value 81 / **-3 dB**. The generic X1E machine driver explicitly describes that 81 limit as a temporary speaker-damage safety cap until active protection exists; SP11 now has active Windows-matched protection. The common first-MCLK pre-state is `0x00`, Windows uses the same `qcadsp8380.mbn`, and fresh Windows qcadcm tracing shows no host hardware-resource table applying a WSA-macro gain write. Zero dB is therefore the leading Windows-equivalent hypothesis but remains indirect because Windows's WSA macro is not APPS-readable and firmware-internal runtime writes are still possible. See `docs/findings/2026-08-15-WSA-ACTIVE-RUNTIME-PRODUCER-STATE.md`.
