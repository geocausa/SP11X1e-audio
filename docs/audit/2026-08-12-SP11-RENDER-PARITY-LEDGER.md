...
- [ ] **AMBER H03 — quiet CSR-off is now localized to WSA8845 prerequisite ordering before SoundWire/PA, with v21 the first stable repeat-cycle improvement.** The fixed-geometry oracle remains decisive: Windows active CSR-off `1.825e-5` median steady diff-RMS; Linux CPS-v3 CSR-on `1.862e-5`; v5 CSR-off `6.765e-4` (~37x Windows). Simple DRE-history candidates v9-v11 are closed; coherent v13 Windows analog-tail init materially improved cycle 1 (`3.175e-4`) but collapsed on cycle 2 (`3.927e-3`). SoundWire topology retention (v15), Windows-style frame-stop completion (v16), CGCR-reset removal (v17), and a physically matched ~0.627-s Windows-like clock-stop boundary (v18/v19) are all insufficient. v21 instead replays the exact audited Windows positions 44-63 analog tail in speaker `hw_params` **before SoundWire add/prepare** on every cycle, while retaining exact v13 transport/producer bytes. Repeat cycles remain stable and better than v5: cycle B `5.666e-4`, cycle C `4.804e-4` (0.71x v5), eliminating the v13 catastrophic repeat-cycle degradation but still ~26x Windows. Read-only write tracing proves Linux then applies gain/DRE/current/PDM state about 10 ms after the tail and the PA transaction ~50 ms after it. The remaining evidence-backed gap is therefore the Windows positions 1-43 prerequisite ordering—especially DRE/watchdog/Class-H state that Windows establishes before positions 44-63—not another SoundWire timer or isolated final register value. See `docs/findings/2026-08-17-SOUNDWIRE-500MS-V18-V19-CLOSURE.md` and `docs/findings/2026-08-17-WSA-ANALOG-TAIL-PRE-SDW-REARM-V21.md`.
...
1. **H03 — refine v21's pre-SoundWire prerequisite ordering:** v21 is the first stable repeat-cycle CSR-off direction and must remain the comparison base. Do not reopen SoundWire timer/topology tuning: v19 physically matched the Windows clock-stop timing family and still failed. Use the v21 register-write trace to move only Windows-proven prerequisite state that Linux currently establishes after the analog tail to the pre-SoundWire boundary. Preserve the stored DRE gain field unless separately justified; full `DRE_CTL_1=0` route-state experiments were noisy. First gate remains 1% muted digital zero, repeated cycles, no music/chirp until the noise floor improves reproducibly.
