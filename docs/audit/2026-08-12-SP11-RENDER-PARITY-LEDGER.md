...
- [ ] **AMBER H03 — the remaining amp gap is now physically localized to quiet CSR-off initialization/state, not final DRE value or ordinary PA ordering.** Windows initializes `DRE_CTL_1=0x00`, leaves it untouched across ordinary PA cycles, and is physically quiet under a proven active non-zero stream. A fixed-geometry four-way SP7 oracle gives median steady diff-RMS: Windows `1.825e-5`; Linux CPS-v3 CSR-on `1.862e-5` (`1.02x` Windows); Linux v5 CSR-off `6.765e-4` (`37.1x`); Linux v8 CSR-off `7.909e-4` (`43.3x`). The Windows control uses a strong one-second 997-Hz marker followed by eleven seconds of continuous 997 Hz at -80 dBFS, so quiet Windows output cannot be explained by exact-zero sample gating. v5 source-to-binary provenance is independently re-verified byte-for-byte. Old v6 remains provenance-invalid. v7 remains rejected after operator-observed active static. v8's original “introduced zero-stream noise” rationale is **reclassified**: v5 already has the same noise class; v8 is merely ~17% worse and has no promotion case. v8 nevertheless proved that Linux can reproduce the exact Windows ordinary PA transaction with zero `DRE_CTL_1` writes and still remain noisy, closing that runtime-write hypothesis. The original qcaucd FIFO has now been decoded as a complete **63-write-per-amp, left/right-identical Windows codec initialization transaction**. It shows Windows establishes `DRE_IDLE=0x0f`, `PDM_WD=1`, `DRE0=0xf0`, `DRE1=0`, `GAIN_RAMPING_MIN=0x0e`, plus deliberate `CKWD 0x1b->0x13` and `CURRENT_LIMIT 0xd4->0x44` transitions during board/supply initialization. Linux reaches several of those same final values only later during gain setup or speaker POST_PMU. The leading gap is therefore earlier WSA8845 initialization/latch history. Do not sweep DRE values or re-arm v7/v8/v9/v10/v11. v9 then isolated the earlier Linux-only no-COMP `CSR_GAIN_EN=1` transition while keeping v5 values/timing elsewhere; its muted zero-stream median diff-RMS is `6.6426e-4` (36.4x Windows, 98.2% of v5), so the early CSR enable history is also closed as the primary cause and v9 is rejected without program-audio escalation. v10 then moved the Windows-proven `DRE_CTL_0=0xf0` into SP11 supply initialization; its zero-stream median diff-RMS is `6.8532e-4` (37.5x Windows, 1.013x v5), so DRE0 init timing alone is also closed and v10 is rejected. v11 then added the otherwise-missing explicit physical Windows `DRE_CTL_1=0x00` init write while keeping v5 behavior afterward; its zero-stream median diff-RMS is `6.8649e-4` (37.6x Windows, 1.015x v5), so that init transaction alone is also closed. See `docs/findings/2026-08-16-CSR-OFF-ACTIVE-NOISE-ORACLE.md`, `docs/findings/2026-08-16-WINDOWS-WSA8845-FULL-INIT-HISTORY.md`, and `artifacts/reviewed/2026-08-16-csr-off-active-noise-fourway.json`.
...
1. **H03 — move from DRE history into board/supply analog init transitions:** v9-v11 close the simple DRE-history candidates individually. Next rank and isolate Windows `CURRENT_LIMIT 0xd4->0x44` versus `CKWD_CTL_1 0x1b->0x13`; both occur inside qcaucd's board/supply configuration function and differ materially from Linux init history. Any candidate must preserve v5 otherwise and first pass the muted digital-silence oracle.
