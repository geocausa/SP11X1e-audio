...
- [x] **GREEN H05 — VISENSE / VI mixers reproduce the Windows slave width.** The corrected `sp11-audio-visense-parity` boot loaded WSA884x srcversion `782FC79EBBA505E52A2AE88`; both amplifiers requested DP5 ChannelEnable `0x03` at 8 kHz, matching the qcaucd slave FIFO, master-port-10/11 runtime tuples and static templates. VI feedback, SP/SPVI enable and bounded playback succeeded without PA fault, XRUN or port conflict, and both amplifiers returned to runtime suspend. Physical tonal judgment remains H07, not part of this transport result.
...
1. **W03/H07 — judge the now-live Windows-width protection path physically:** corrected one-shot `sp11-audio-visense-parity` loaded DP5 `0x03` on both amplifiers; VI/CPS, SP/SPVI, graph startup, bounded playback and suspend gates pass. The retained Windows hardware transaction is transport/profile closed and DP4 remains correctly unscheduled. Compare tonal balance, body, spatial presentation and high-level behavior with the previous Linux build and Windows; do not infer acoustic parity from transport success alone.
