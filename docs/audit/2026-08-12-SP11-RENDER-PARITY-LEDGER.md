...
1. **H03 — finish the SoundWire master low-power/wake parity map:** v16 closes one real contributor: Windows and Linux both use Mode-0 broadcast `SCP_CTRL=CLK_STOP_NOW`, but Windows waits for `SWRM_COMP_STATUS[0]` to clear before gating the lower clock path. Adding that bounded wait reduced the repeat-cycle noise from v13-cycle2/v15 levels to `2.106e-3`, but remains ~115x Windows, so it is necessary-direction evidence rather than closure. Next classify Windows qcaucd Resource Hub wake votes against Linux `LPASS_AUDIO_SWR_WSA_CGCR` reset before deciding whether the next isolation is CGCR-reset behavior or the independently proven 500-ms-vs-3000-ms clock-stop timing. See `docs/findings/2026-08-17-SOUNDWIRE-POST-STOP-FRAMEGEN-WAIT-V16-REJECTION.md`.
