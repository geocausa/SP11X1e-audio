...
> **2026-08-08 continuation:** the state-pinned Aug-7 Windows run materially
> narrows the remaining parity gap. The normal `peak-level=0` VLLDP limiter is
> directly observed attenuating, and the inner VLLDP input staging is already
> ~3 dB hotter than the deterministic source. Start current integration work at
> `docs/audit/2026-08-08-DOLBY-INTEGRATION-STATUS.md` and
> `docs/findings/2026-08-07-STATE-PINNED-DYNAMIC-VLLDP-PRELIMITER-DRIVE.md`.
> Historical Aug-5/6 conclusions remain provenance, not the final open-problem
> statement.

**Status:** Aug-5 canonical model, continued by the Aug-8 integration status above.
...
Publishable integration is carried forward without vendor/build artifacts; use the Aug-8 integration status as the current engineering entry point.
